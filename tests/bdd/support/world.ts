...
const PRISMA_JINGLE_DB_SCRIPT_PATH = join(REPO_ROOT, "scripts/run-prisma-jingle-db.mjs")
...
async function resolveWindowByKind(
  ...
  const windowSummaries = await Promise.all(
    electronApp.windows().map(async (page) => ({
      kind: await readWindowKind(page),
      title: await page.title().catch((error: unknown) => `[title error: ${String(error)}]`),
      url: page.url()
    }))
  )

  throw new Error(
    `Window "${windowKind}" did not finish bootstrapping within 30 seconds. Open windows: ${JSON.stringify(windowSummaries)}`
  )
}
...
async function prepareDatabase(jingleHome: string): Promise<void> {
  const databasePath = join(jingleHome, "jingle.sqlite")

  try {
    await execFileAsync(process.execPath, [PRISMA_JINGLE_DB_SCRIPT_PATH, "migrate", "deploy"], {
      cwd: REPO_ROOT,
      env: {
        ...process.env,
...
export class JingleWorld extends World {
  ...
  prepareJingleHome(): string {
    if (!this.jingleHome) {
      this.jingleHome = mkdtempSync(join(tmpdir(), "jingle-bdd-"))
      process.env.JINGLE_HOME = this.jingleHome
    }

    return this.jingleHome
  }

  useScriptedAgentRuntime(): void {
    if (this.electronApp) {
      throw new Error("Scripted agent runtime must be selected before launching Jingle.")
    }
  ...
  setExtensionRuntimeFixture(key: string, value: unknown): void {
    if (this.electronApp) {
      throw new Error("Extension runtime fixtures must be configured before launching Jingle.")
    }
  ...
  getJingleHome(): string {
    if (!this.jingleHome) {
      throw new Error("BDD JINGLE_HOME is not available before launch.")
    }
  ...
  async getPageByKind(windowKind: BddWindowKind): Promise<Page> {
    if (!this.electronApp) {
  ...
  async isWindowVisible(windowKind: BddWindowKind): Promise<boolean> {
    if (!this.electronApp) {
      throw new Error("BDD Electron app is not available. Launch the app before using page steps.")
    }
  ...
  async closeApp(): Promise<void> {
    ...
    if (this.jingleHome) {
      rmSync(this.jingleHome, { force: true, recursive: true })
      this.jingleHome = null
    }
...
setWorldConstructor(JingleWorld)
