...
功能: Workspace 主进程契约
  ...
  场景: Main 窗口切换线程时 workspace picker 跟随当前线程
    假如 Openwork 桌面应用已启动
    ...
    当 我直接打开 Main 历史窗口
    并且 我在 Main 窗口选择标题为 "BDD Workspace First UI" 的线程
    那么 Main 窗口 workspace picker 路径应为标题 "BDD Workspace First UI" 的线程 workspace
    当 我在 Main 窗口选择标题为 "BDD Workspace Second UI" 的线程
    那么 Main 窗口 workspace picker 路径应为标题 "BDD Workspace Second UI" 的线程 workspace
