...
async function listLauncherHistory(world: OpenworkWorld): Promise<LauncherHistoryItem[]> {
...
async function removeLauncherHistory(world: OpenworkWorld, itemId: string): Promise<void> {
  const page = await world.getPageByKind("launcher")
...
Given(
  ...
  function (this: OpenworkWorld, pinnedTitle: string, recentTitle: string) {
...
Given(
  ...
  function (this: OpenworkWorld, olderTitle: string, recentTitle: string) {
    const openworkHome = this.prepareOpenworkHome()
...
When("我读取 launcher history 列表", async function (this: OpenworkWorld) {
