...
interface ModelProviderWorld extends JingleWorld {
  modelProviderState?: ModelProviderScenarioState
}

const MODEL_PROVIDER_BDD_HOME = mkdtempSync(join(tmpdir(), "jingle-bdd-model-provider-"))
const originalJingleHome = process.env.JINGLE_HOME
const originalFetch = globalThis.fetch
const requireFromStep = createRequire(__filename)

process.env.JINGLE_HOME = MODEL_PROVIDER_BDD_HOME
