...
async function listLocalStart(world: JingleWorld): Promise<LocalStartItem[]> {
  const page = await world.getPageByKind("launcher")
...
async function recordLocalStartUse(world: JingleWorld, itemId: string): Promise<LocalStartItem> {
  const page = await world.getPageByKind("launcher")
...
async function removeLocalStart(world: JingleWorld, itemId: string): Promise<void> {
  const page = await world.getPageByKind("launcher")
...
function getLatestLocalStartItems(world: JingleWorld): LocalStartItem[] {
...
When(
  ...
  async function (this: JingleWorld, title: string, relativePath: string) {
    const itemPath = join(this.getJingleHome(), relativePath)
...
When("我读取 local start 列表", async function (this: JingleWorld) {
...
Then("local start 列表包含标题为 {string} 的项", function (this: JingleWorld, title: string) {
  const items = getLatestLocalStartItems(this)

  expect(items.some((item) => item.title === title)).toBe(true)
})

Then("local start 列表不包含标题为 {string} 的项", function (this: JingleWorld, title: string) {
  const items = getLatestLocalStartItems(this)
...
Then(
  ...
  function (this: JingleWorld, title: string, useCount: number) {
...
Then(
  ...
  function (this: JingleWorld, count: number) {
