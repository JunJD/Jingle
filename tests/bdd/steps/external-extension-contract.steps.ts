...
function createGitHubNotification(): GitHubNotification {
  return {
    id: "101",
    reason: "mention",
    repositoryFullName: "jingle/runtime-contract",
    subjectType: "Issue",
    title: "BDD GitHub Notification",
    unread: true,
    updatedAt: "2026-05-10T00:00:00.000Z",
    url: "https://github.com/jingle/runtime-contract/issues/101"
  }
}
...
Given(
  ...
  function (this: JingleWorld, responseText: string) {
    this.setExtensionRuntimeFixture("aiResponseText", responseText)
  }
)

Given("BDD extension runtime RPC 使用测试数据", function (this: JingleWorld) {
...
Given("BDD Notion API 使用连接后的测试数据", async function (this: JingleWorld) {
...
Given("Notion extension 已连接到 BDD Notion API", async function (this: JingleWorld) {
...
When("我在 Launcher 中搜索 BDD Notion 快速剪藏 URL", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
...
When(
  ...
  async function (this: JingleWorld, title: string, value: string) {
    const page = await this.getPageByKind("launcher")
...
When("我执行当前 runtime primary action", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
...
When(
  ...
  async function (this: JingleWorld, actionTitle: string) {
    const page = await this.getPageByKind("launcher")
...
Then("runtime form 当前标题为 {string}", async function (this: JingleWorld, title: string) {
  const page = await this.getPageByKind("launcher")
...
Then("runtime list 当前标题为 {string}", async function (this: JingleWorld, title: string) {
  const page = await this.getPageByKind("launcher")
  const list = getRuntimeListRoot(page)
...
Then("runtime list 展示条目 {string}", async function (this: JingleWorld, title: string) {
  const page = await this.getPageByKind("launcher")
  const list = getRuntimeListRoot(page)
...
Then(
  ...
  async function (this: JingleWorld, fieldId: string, text: string) {
    const page = await this.getPageByKind("launcher")
    const form = getRuntimeFormRoot(page)
...
Then(
  ...
  async function (this: JingleWorld, expectedPrompt: string) {
    const lastPrompt = await this.evaluateInMain(() => {
      const probe = (globalThis as typeof globalThis & {
        __JINGLE_BDD_EXTENSION_RUNTIME__?: ExtensionRuntimeBddProbe
      }).__JINGLE_BDD_EXTENSION_RUNTIME__
      if (probe === undefined || probe.lastAiAsk === undefined) {
        return undefined
      }

      return probe.lastAiAsk.prompt
    }, null)
...
Then(
  ...
  async function (this: JingleWorld, expectedCapability: string) {
    const lastCapability = await this.evaluateInMain(() => {
      const probe = (globalThis as typeof globalThis & {
        __JINGLE_BDD_EXTENSION_RUNTIME__?: ExtensionRuntimeBddProbe
      }).__JINGLE_BDD_EXTENSION_RUNTIME__
      if (probe === undefined || probe.hostRequests === undefined || probe.hostRequests.length === 0) {
        return undefined
      }

      return probe.hostRequests[probe.hostRequests.length - 1]?.capability
    }, null)
...
Then("BDD Notion API 收到 search 请求", function (this: JingleWorld) {
...
Then("BDD Notion API 收到快速剪藏页面读取请求", function (this: JingleWorld) {
  const requests = this.getScenarioObject<NotionMockRequest[]>("notionMock.requests")
...
Then(
  ...
  async function (this: JingleWorld, pageId: string, text: string) {
    const requests = this.getScenarioObject<NotionMockRequest[]>("notionMock.requests")
...
Then("系统剪贴板文本应为 {string}", async function (this: JingleWorld, expectedText: string) {
...
Then(
  ...
  async function (this: JingleWorld, extensionName: string) {
