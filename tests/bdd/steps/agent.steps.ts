...
interface AgentBddStream {
  cleanup: (() => void) | null
  events: JingleRuntimeEventBatch<AgentThreadEvent>[]
}
...
interface AgentBddWindow extends Window {
  __jingleAgentBdd?: AgentBddStore
  api: {
    agent: {
...
function getLatestThreadId(world: JingleWorld): string {
  return world.getScenarioValue("agent.latestThreadId")
}

function getLatestStreamKey(world: JingleWorld): string {
  return world.getScenarioValue("agent.latestStreamKey")
}

async function getLauncherPage(world: JingleWorld): Promise<Page> {
  return world.getPageByKind("launcher")
}

async function getThreadStatus(world: JingleWorld): Promise<string | null> {
...
async function getAgentThreadData(world: JingleWorld): Promise<AgentThreadDataSnapshot> {
  const page = await getLauncherPage(world)
  const threadId = getLatestThreadId(world)
...
async function getStreamEvents(
  world: JingleWorld
): Promise<JingleRuntimeEventBatch<AgentThreadEvent>[]> {
  const page = await getLauncherPage(world)
  const streamKey = getLatestStreamKey(world)

  return page.evaluate((inputStreamKey) => {
    const store = (window as unknown as AgentBddWindow).__jingleAgentBdd
    return store?.streams[inputStreamKey]?.events ?? []
  }, streamKey)
}

function getLatestStreamPendingApprovalId(
  events: JingleRuntimeEventBatch<AgentThreadEvent>[]
): string | null {
  return (
    events
...
async function waitForStreamEventType(world: JingleWorld, type: string): Promise<void> {
  const page = await getLauncherPage(world)
  const streamKey = getLatestStreamKey(world)
...
async function startStreamAgent(
  ...
  await page.evaluate(
    ({ decision, message, threadId: inputThreadId, streamKey: inputStreamKey }) => {
      const targetWindow = window as unknown as AgentBddWindow
      targetWindow.__jingleAgentBdd ??= { streams: {} }

      const events: JingleRuntimeEventBatch<AgentThreadEvent>[] = []
      const cleanup = targetWindow.api.agent.connectThreadEvents(inputThreadId, (batch) => {
        events.push(batch)
      })

      targetWindow.__jingleAgentBdd.streams[inputStreamKey] = {
        cleanup,
        events
      }
...
After(async function (this: JingleWorld) {
  try {
    ...
    await page.evaluate(() => {
      ...
      delete (window as unknown as AgentBddWindow).__jingleAgentBdd
    })
  } catch {
...
Given("Jingle 桌面应用已使用脚本化 agent runtime 启动", async function (this: JingleWorld) {
...
When(
  ...
  async function (this: JingleWorld, title: string) {
    const page = await getLauncherPage(this)
    const workspacePath = join(this.getJingleHome(), "agent-workspace", title)
...
When("我取消最新 agent 线程", async function (this: JingleWorld) {
  const page = await getLauncherPage(this)
...
When("我通过 agent resume 批准最新待审批请求", async function (this: JingleWorld) {
...
Then("最新 agent stream 应收到 done", async function (this: JingleWorld) {
  await waitForStreamEventType(this, "run.finished")
})

Then("最新 agent stream 不应收到 done", async function (this: JingleWorld) {
  const page = await getLauncherPage(this)
...
Then("最新 agent stream 应收到取消完成事件", async function (this: JingleWorld) {
  await expect
    .poll(async () =>
      (await getStreamEvents(this)).some((batch) =>
...
Then(
  ...
  async function (this: JingleWorld, expectedText: string) {
    await expect
...
Then("最新 agent stream 应进入长任务", async function (this: JingleWorld) {
  await expect
    .poll(async () => JSON.stringify(await getStreamEvents(this)))
    .toContain("scripted agent long task started")
})

Then("最新 agent stream 应收到 HITL 中断", async function (this: JingleWorld) {
  await expect
    .poll(async () =>
      (await getStreamEvents(this)).some((batch) =>
...
Then("最新 agent stream HITL 请求 id 应与 runtime state 一致", async function (this: JingleWorld) {
  await expect
    .poll(async () => {
...
Then(
  ...
  async function (this: JingleWorld, toolName: string) {
    await expect
...
Then("最新 agent runtime state 待审批请求应为空", async function (this: JingleWorld) {
  await expect.poll(async () => (await getRuntimeState(this)).pendingApproval).toBeNull()
})

Then("最新 agent thread data 消息数应为 {int}", async function (this: JingleWorld, expectedCount: number) {
...
Then(
  ...
  async function (this: JingleWorld, expectedUserCount: number, expectedAssistantCount: number) {
    await expect
      .poll(async () => {
...
Then(
  ...
  async function (this: JingleWorld, expectedStatus: string) {
