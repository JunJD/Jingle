...
When("我在 Todo List 中创建一条新的测试待办", async function (this: JingleWorld) {
...
When("我在 Todo List 输入框中输入新的测试待办标题", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
  const input = getTodoListInput(page)
  const title = `BDD todo ${randomUUID()}`
...
When("我在 Todo List 输入框按下 Enter", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
  const input = getTodoListInput(page)
...
When("我在当前 Launcher surface 打开动作面板", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
...
When("我在原生动作面板中向下移动一次", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")

  await page.keyboard.press("ArrowDown")
})

When("我执行当前选中的原生动作", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")

  await page.keyboard.press("Enter")
})

When("我关闭原生动作面板", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")

  await page.keyboard.press("Escape")
})

Then("Launcher 当前命令归属为 {string}", async function (this: JingleWorld, ownerId: string) {
  const page = await this.getPageByKind("launcher")
...
Then("Todo List 展示刚创建的待办", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
  const todoListRoot = getTodoListRoot(page)
...
Then("Launcher 原生动作面板可见", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")

  await expect(page.locator('[data-surface="native-action-panel"]')).toBeVisible()
})

Then("Launcher 原生动作面板已隐藏", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")

  await expect(page.locator('[data-surface="native-action-panel"]')).toHaveCount(0)
})

Then("Todo List 已进入搜索模式", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
  const todoListRoot = getTodoListRoot(page)
