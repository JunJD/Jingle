...
async function readBootstrapState(world: JingleWorld): Promise<{
...
async function readShortcutSettings(world: JingleWorld): Promise<ShortcutSettings> {
  const page = await world.getPageByKind("launcher")
...
async function readResolvedBindings(world: JingleWorld): Promise<ResolvedShortcutBinding[]> {
  const page = await world.getPageByKind("launcher")
...
async function readGlobalAvailability(world: JingleWorld): Promise<GlobalShortcutAvailability[]> {
  const page = await world.getPageByKind("launcher")
...
When("我读取 shortcuts bootstrap 状态", async function (this: JingleWorld) {
...
When("我读取 shortcuts 当前设置", async function (this: JingleWorld) {
...
When("我开始监听 shortcuts settingsChanged 事件", async function (this: JingleWorld) {
...
When(
  ...
  async function (this: JingleWorld, shortcutLabel: string) {
...
When("我重置 shortcuts 设置", async function (this: JingleWorld) {
  const settings = await setShortcutSettings(this, { overrides: [] })

  this.setScenarioValue("shortcuts.currentSettings", JSON.stringify(settings))
})

When("我读取 shortcuts global availability", async function (this: JingleWorld) {
  const records = await readGlobalAvailability(this)

  this.setScenarioValue("shortcuts.globalAvailability", JSON.stringify(records))
})

Then("shortcuts bootstrap 设置应等于当前设置", function (this: JingleWorld) {
...
Then(
  ...
  function (this: JingleWorld, commandId: string, source: ShortcutSource) {
...
Then(
  ...
  function (this: JingleWorld, expectedChord: string) {
...
Then(
  ...
  async function (this: JingleWorld, expectedChord: string) {
    const page = await this.getPageByKind("launcher")
...
Then("shortcuts 当前设置不包含 launcher.toggle override", function (this: JingleWorld) {
  const settings = JSON.parse(
    this.getScenarioValue("shortcuts.currentSettings")
  ) as ShortcutSettings
...
Then(
  ...
  function (this: JingleWorld, commandId: string) {
