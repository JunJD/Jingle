...
async function readLauncherAiStreamingReplyState(page: Page): Promise<{
  ...
  return {
    ...
    text: (await streamingAssistant.innerText()).trim(),
    turnStreaming: true
  }
}

async function readCurrentLauncherAiThreadId(page: Page): Promise<string | null> {
  const threadId = await getLauncherAiSurface(page)
    .locator("[data-launcher-ai-main]")
    .first()
    .getAttribute("data-launcher-ai-thread-id")
...
async function getLatestLauncherAiThreadData(
  ...
  if (currentThreadId) {
    return page.evaluate(async (threadId) => {
      ...
      ).api.threads.getAgentThreadData(threadId)
    }, currentThreadId)
  }
...
When("我重新启动 Jingle 桌面应用", async function (this: JingleWorld) {
  await this.restartApp()
})

Given("存在标题为 {string} 的历史线程", async function (this: JingleWorld, title: string) {
...
Given(
  "存在标题为 {string} 的 Launcher AI 历史线程",
...
Given(
  ...
  async function (this: JingleWorld, title: string, message: string) {
    const { threadId } = await seedHistoryThreadFixture({
      messages: [message],
      title
    })
...
Given(
  "存在标题为 {string} 且包含历史消息 {string} 的 Launcher AI 历史线程",
  async function (this: JingleWorld, title: string, message: string) {
    const { threadId } = await seedHistoryThreadFixture({
      messages: [message],
      metadata: { source: AI_THREAD_SOURCE },
...
Then("Launcher 窗口可用", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")

  await expect(page).toHaveTitle(/jingle/i)
  expect(page.isClosed()).toBe(false)
})

Then("渲染进程标识为 Launcher 窗口", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
...
Then("Launcher React 根节点已完成渲染", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
...
Then("Launcher 首页展示了可执行结果", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
...
Then(
  ...
  async function (this: JingleWorld, fragment: string) {
...
When("我执行当前选中的 Launcher 结果", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
  const input = getLauncherInput(page, "home")
...
When("我打开名为 {string} 的 Launcher 结果", async function (this: JingleWorld, title: string) {
  const page = await this.getPageByKind("launcher")
  const result = page
    .locator(".launcher-result-row")
    .filter({ has: page.getByText(title, { exact: true }) })
...
When("我直接打开 Main 历史窗口", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
...
When("我在 Launcher 中按下 Escape", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")

  await page.keyboard.press("Escape")
})

When("我在 Launcher 首页按下 Escape", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")

  await page.keyboard.press("Escape")
})

When("我在 Launcher 首页按下 Tab", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
  const input = getLauncherInput(page, "home")
...
When("我在 Launcher 首页按下 ArrowDown", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")

  await page.keyboard.press("ArrowDown")
})

When("我在 Launcher 首页按下 ArrowUp", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")

  await page.keyboard.press("ArrowUp")
})

When("我在 Launcher AI 输入框按下 Backspace", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
...
When("我从 Launcher 打开设置窗口", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
...
When("我通过 API 打开 Settings 窗口", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
...
Then("Launcher 界面切换到 {string}", async function (this: JingleWorld, surface: string) {
  const page = await this.getPageByKind("launcher")
...
Then("Launcher 输入框包含 {string}", async function (this: JingleWorld, query: string) {
  const page = await this.getPageByKind("launcher")

  await expect.poll(() => readLauncherVisibleInputValue(page)).toBe(query)
})

Then("Launcher AI 输入状态会进入 pending", async function (this: JingleWorld) {
  const page = await this.getPageByKind("launcher")
...
Then(
  ...
  async function (this: JingleWorld, text: string) {
    const page = await this.getPageByKind("launcher")

    await expect
      .poll(async () => {
        const state = await readLauncherAiStreamingReplyState(page)
        return {
          assistantStreaming: state.assistantStreaming,
          textVisible: state.text.includes(text),
          turnStreaming: state.turnStreaming
        }
      })
      .toEqual({
        assistantStreaming: true,
...
Then(
  "Launcher AI 当前回复在完成前依次显示流式文本",
  async function (this: JingleWorld, table: DataTable) {
    const page = await this.getPageByKind("launcher")
    const expectedTexts = table
      .raw()
      .flat()
      .map((text) => text.trim())
      .filter(Boolean)
    ...
    await expect
      .poll(
        async () => {
          const state = await readLauncherAiStreamingReplyState(page)
          if (!state.turnStreaming || !state.assistantStreaming) {
            return matchedCount
          }

          while (
            matchedCount < expectedTexts.length &&
            state.text.includes(expectedTexts[matchedCount]!)
          ) {
            matchedCount += 1
          }
...
Then(
  ...
  async function (this: JingleWorld, durationMs: number, message: string) {
    const page = await this.getPageByKind("launcher")
    const aiSurface = getLauncherAiSurface(page)
...
Then(
  ...
  async function (this: JingleWorld, expectedCount: number, text: string) {
    const page = await this.getPageByKind("launcher")
...
Then("Launcher 首页当前选中结果为 {string}", async function (this: JingleWorld, title: string) {
  const page = await this.getPageByKind("launcher")
...
Then("Launcher 窗口已隐藏", async function (this: JingleWorld) {
  await expect.poll(() => this.isWindowVisible("launcher")).toBe(false)
})

Then("Settings 窗口可用", async function (this: JingleWorld) {
...
Then("Main 窗口可用", async function (this: JingleWorld) {
...
When("我切换到 Settings 快捷键页", async function (this: JingleWorld) {
  const page = await this.getPageByKind("settings")

  await page.locator('[data-settings-tab="shortcuts"]').click()
})

When("我开始编辑 launcher.toggle 快捷键", async function (this: JingleWorld) {
  const page = await this.getPageByKind("settings")
...
When("我录制新的 launcher.toggle 快捷键", async function (this: JingleWorld) {
  const page = await this.getPageByKind("settings")
...
When("我保存 launcher.toggle 快捷键", async function (this: JingleWorld) {
  const page = await this.getPageByKind("settings")

  await page.locator('[data-shortcut-save="launcher.toggle"]').click()
})

Then("Settings 展示 launcher.toggle 快捷键", async function (this: JingleWorld) {
  const page = await this.getPageByKind("settings")
...
Then("Settings 为 launcher.toggle 显示可用的全局注册状态", async function (this: JingleWorld) {
  const page = await this.getPageByKind("settings")
