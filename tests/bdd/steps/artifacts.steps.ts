...
async function readArtifacts(world: OpenworkWorld, threadId: string): Promise<ArtifactRecord[]> {
...
Given(
  ...
  async function (this: OpenworkWorld, title: string, content: string) {
    const fixture = await seedManagedFileArtifactFixture({
      ...
      mimeType: "text/plain",
      openworkHome: this.getOpenworkHome(),
      title
    })
...
Given(
  ...
  async function (this: OpenworkWorld, title: string, bytes: string) {
    const fixture = await seedManagedFileArtifactFixture({
...
When("我读取当前 artifact 线程的 artifacts 列表", async function (this: OpenworkWorld) {
...
When("我读取最新 artifact 的文本内容", async function (this: OpenworkWorld) {
...
When("我读取最新 artifact 的二进制内容", async function (this: OpenworkWorld) {
  const artifactId = this.getScenarioValue("artifacts.latestArtifactId")
...
When(
  ...
  async function (this: OpenworkWorld, action: ArtifactActionId) {
    const artifactId = this.getScenarioValue("artifacts.latestArtifactId")
...
Then(
  ...
  function (this: OpenworkWorld, title: string, kind: ArtifactRecord["kind"]) {
...
Then("最新 artifact 文本读取结果应成功", function (this: OpenworkWorld) {
...
Then(
  ...
  function (this: OpenworkWorld, content: string) {
    const result = JSON.parse(
      this.getScenarioValue("artifacts.latestTextReadResult")
    ) as ArtifactFileReadResult
...
Then("最新 artifact 二进制读取结果应成功", function (this: OpenworkWorld) {
  const result = JSON.parse(
...
Then(
  ...
  function (this: OpenworkWorld, type: ArtifactActionResolution["type"]) {
    const result = JSON.parse(
...
Then("最新 artifact open 结果 uri 应为托管 artifact 路径", function (this: OpenworkWorld) {
  const result = JSON.parse(
    this.getScenarioValue("artifacts.latestOpenResult")
  ) as ArtifactActionResolution
