...
test("use-with shell excludes commands that require search arguments from generic fallbacks", () => {
  const timerCommand: LauncherIndexedCommand = {
    address: {
      commandName: "caffeinateFor",
      extensionName: "coffee",
      kind: "extension-command"
    },
    description: "Keep the Mac awake for a duration",
    iconName: "cup",
    keywords: ["coffee", "duration"],
    ownerTitle: "Coffee",
  ...
  const items = buildLauncherUseWithShellItems({
    commands: [timerCommand, filesCommand],
    copy,
    intentItems: [],
    query: "coffee"
  })
  ...
  assert.deepEqual(
    items.map((item) => item.id),
    ["use-with:files:search-files:coffee"]
  )
})
