...
test("packaged extension commands keep asset icons with glyph fallbacks in launcher results", () => {
  ...
  assert.match(initialIconMarkup, /jingle-extension-asset:\/\/github\/assets\/icon\.svg/)
})
...
test("installed Notion commands are available through renderer catalog projection", () => {
  ...
  assert.match(initialIconMarkup, /<img/)
  assert.match(initialIconMarkup, /jingle-extension-asset:\/\/notion\/assets\/notion-logo\.png/)
})
...
test("installed Figma Files commands are available through renderer catalog projection", () => {
  ...
  assert.match(
    initialIconMarkup,
    /jingle-extension-asset:\/\/figma-files\/assets\/command-icon\.png/
  )
})
...
test("Notion key aliases do not resolve without an explicit command owner", () => {
  assert.equal(
    resolveLauncherCommand({
      altKey: false,
      ctrlKey: false,
...
test("quicklink search results can open extension commands with launch context", () => {
  const command = parseExtensionQuicklinkCommandUrl(
    "jingle://extensions/HenriChabrand/notion/create-database-page?launchContext=%7B%22defaults%22%3A%7B%22title%22%3A%22Spec%22%7D%7D"
  )
