...
test("extension CLI builds bundled trusted extensions as installed runtime packages", async () => {
  const rootDir = await mkdtemp(join(tmpdir(), "openwork-installed-packages-"))
  try {
    await execFileAsync(
      process.execPath,
      [
    ...
    assert.deepEqual(
      registry.listManifests("darwin").map((manifest) => manifest.name),
      ["apple-reminders", "figma-files", "github", "notion"]
    )
    ...
    assert.doesNotMatch(mainBundle, /Openwork extension runtime React bridge is not installed/)
    assert.doesNotMatch(mainBundle, /openwork-runtime-shim:react/)
    assert.match(
...
test("extension CLI dev fails fast when the initial build fails", async () => {
  const sourceRoot = await mkdtemp(join(tmpdir(), "openwork-bad-extension-source-"))
  const outputRoot = await mkdtemp(join(tmpdir(), "openwork-bad-extension-out-"))
...
test("extension CLI rejects function search adapters in installable runtime metadata", async () => {
  const sourceRoot = await mkdtemp(join(tmpdir(), "openwork-function-metadata-source-"))
  const outputRoot = await mkdtemp(join(tmpdir(), "openwork-function-metadata-out-"))
...
test("installed extension provider loads a valid descriptor package", async () => {
  const rootDir = await mkdtemp(join(tmpdir(), "openwork-installed-extension-"))
  try {
...
async function writeInstalledExtensionFixture(
  ...
  await writeFile(
    join(packageRoot, "openwork.extension.json"),
    JSON.stringify({
      assets: "./assets",
      id: "sample",
      main: "./dist/main.mjs",
