...
test("extension CLI builds bundled trusted extensions as installed runtime packages", async () => {
  ...
  try {
    ...
    assert.ok(notionPackage.main)
    const installedNotionMainDefinition = await loadExtensionMainDefinition(notionPackage.main)
    assert.ok(installedNotionMainDefinition.tools?.length)
