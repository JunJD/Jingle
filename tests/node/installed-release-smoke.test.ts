...
test("release workflow keeps candidates build-only and publishes only verified tag assets", () => {
  ...
  assert.match(smokeSource, /file\|containerfile/)
  assert.match(smokeSource, /CurrentThreatExecutionStatusID/)
  assert.match(smokeSource, /ThreatStatusID/)
  assert.match(smokeSource, /MatchedResource = \$target/)
