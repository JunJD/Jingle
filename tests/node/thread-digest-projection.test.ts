...
test.before(async () => {
  openworkHome = await mkdtemp(join(tmpdir(), "openwork-thread-digest-"))
...
test("closeRuntime flushes queued thread digest projection", async () => {
  ...
  const { closeRuntime } = await import("../../src/main/agent/runtime")
  const threadId = "thread-digest-close-runtime"

  await createThread(threadId)
  await syncMessageSearchIndexFromSnapshot(threadId, [
    {
      content: JSON.stringify("closeRuntime should flush queued digest work."),
  ...
  const restoreGenerator = setThreadDigestGeneratorForTests(async () => ({
    decisions: [],
    openQuestions: [],
    summary: "closeRuntime flushes queued digest work.",
    topics: ["closeRuntime"]
  }))
  try {
    enqueueThreadDigestProjection(threadId)
