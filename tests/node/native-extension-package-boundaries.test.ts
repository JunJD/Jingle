...
test("native extension package boundary check enforces package entry shape", async () => {
  ...
  try {
    const errors = validateNativeExtensionPackageBoundaries({ repoRoot }).errors.join("\n")
    assert.match(errors, /must declare "name": "@jingle\/extension-fixture"/)
...
test("native extension package boundary check rejects source runtime package imports and declarations", async () => {
  ...
  try {
    ...
    assert.match(errors, /cannot import source runtime package @raycast\/api; use @jingle\/extension-api/)
    assert.match(errors, /cannot import source runtime package @raycast\/utils; use @jingle\/extension-utils/)
  } finally {
    await rm(repoRoot, { force: true, recursive: true })
  }
})
