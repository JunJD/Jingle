...
test("LangSmith API key enables LangSmith tracing without agent runtime wiring", () => {
  ...
  assert.equal(env.LANGSMITH_PROJECT, "openwork-dev")
...
test("observability ignores unsupported provider env because provider selection is hard-coded", () => {
  const env: Record<string, string | undefined> = {
    OPENWORK_OBSERVABILITY_PROVIDER: "unknown"
  }
...
test("agent trace metadata carries Openwork ids without workspace paths", () => {
  assert.deepEqual(
    buildAgentRunTraceMetadata({
      modelId: "gpt-5",
      permissionMode: "auto",
    ...
    }),
    {
      run_id: "run-1",
      thread_id: "thread-1",
      openwork_run_id: "run-1",
      openwork_thread_id: "thread-1",
      openwork_run_source: "invoke",
      openwork_model_id: "gpt-5",
      openwork_permission_mode: "auto"
    }
  )
})
...
test("agent run trace config uses LangChain RunnableConfig tracing fields", () => {
  assert.deepEqual(
    ...
    {
      runName: "openwork.agent.resume",
      tags: ["openwork", "openwork:resume"],
      metadata: {
        run_id: "run-1",
        thread_id: "thread-1",
        openwork_run_id: "run-1",
        openwork_thread_id: "thread-1",
        openwork_run_source: "resume",
        openwork_model_id: "gpt-5",
        openwork_permission_mode: "auto"
      }
    }
  ...
  assert.deepEqual(
    ...
    }),
    {
      ...
      metadata: {
        ...
        openwork_model_id: "claude-test",
        openwork_permission_mode: "ask-to-edit"
      }
    }
  )
})
...
test("dev command env loads local .env without overriding exported env", () => {
  ...
  const tempDir = join(tmpdir(), `openwork-observability-${Date.now()}`)
  mkdirSync(tempDir)
