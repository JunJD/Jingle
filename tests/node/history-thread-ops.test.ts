...
test("activateHistoryThread loads threads and ensures the requested thread is loaded when it becomes available", async () => {
  ...
  assert.deepEqual(loadCalls, ["thread-2"])
})

test("openHistoryThread returns false when the requested thread is still missing after refresh", async () => {
  const store = createHistoryThreadStoreFixture({
...
test("refreshHistoryThreadsAndLoadActive loads the current thread after refreshing threads", async () => {
  const store = createHistoryThreadStoreFixture({
  ...
  const threads = await ops.refreshHistoryThreadsAndLoadActive(async (threadId) => {
    loadCalls.push(threadId)
  })
  ...
  assert.deepEqual(loadCalls, ["thread-active"])
})
