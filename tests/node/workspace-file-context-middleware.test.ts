...
import { createWorkspaceFileContextMiddleware } from "../../src/main/agent/workspace-file-context-middleware"
...
test("workspace file context middleware injects referenced workspace file content", async () => {
  ...
  const middleware = createWorkspaceFileContextMiddleware({
    threadId: "thread-1",
    workspaceService
  })
...
test("workspace file context middleware escapes file context attributes", async () => {
  ...
  const userMessage = new HumanMessage({
    ...
    content: 'Review [@src/"quoted".ts](openwork-workspace-file://src%2F%22quoted%22.ts)'
  })
