...
async function withWorkspace(
  ...
  prefix = "jingle-mutation-predictor-"
): Promise<void> {
...
test("predicts file writes relative to an explicit cwd", async () => {
  await withWorkspace(async (workspacePath) => {
    mkdirSync(path.join(workspacePath, "src"), { recursive: true })
    const predictor = new JustBashMutationPredictor({ workspacePath, timeoutMs: 10_000 })

    const prediction = await predictor.predictExecute("touch nested.txt", { cwd: "src" })

    assert.equal(prediction.status, "predicted")
    assert.equal(prediction.exitCode, 0)
    assert.equal(hasChange(prediction, "src/nested.txt", "create"), true)
  })
})

test("predicts old cd chains using the simulated shell cwd", async () => {
  await withWorkspace(async (workspacePath) => {
    mkdirSync(path.join(workspacePath, "src"), { recursive: true })
    const predictor = new JustBashMutationPredictor({ workspacePath, timeoutMs: 10_000 })

    const prediction = await predictor.predictExecute("cd src && touch legacy.txt")

    assert.equal(prediction.status, "predicted")
    assert.equal(prediction.exitCode, 0)
    assert.equal(hasChange(prediction, "src/legacy.txt", "create"), true)
  })
})
