...
function buildRequest(command: string, cwd?: unknown) {
  return {
    toolName: "execute",
...
test("read-only commands bypass prediction and stay allowed", async () => {
  ...
  assert.equal(decision.reasons?.[0]?.code, "jingle.controlled_shell_allowed")
...
test("predictable mutations pass resolved cwd to the predictor", async () => {
  let predictorCwd: string | null = null
  const provider = createExecuteCommandGuardrailProvider({
    classifier: {
      classify() {
        return buildPolicy({
    ...
    predictor: {
      async predictExecute(_command, options) {
        predictorCwd = options?.cwd ?? null
        return buildPrediction()
      }
    }
  })

  const decision = await provider.evaluate(buildRequest("echo hello > file.txt", "packages/api"))

  assert.equal(decision.allow, true)
  assert.equal(predictorCwd, path.join("/workspace", "packages/api"))
})

test("execute cwd outside the workspace requires explicit approval", async () => {
  ...
  const provider = createExecuteCommandGuardrailProvider({
    ...
    predictor: {
      async predictExecute() {
        throw new Error("predictor should not run for outside-workspace cwd")
      }
    }
  })

  const decision = await provider.evaluate(buildRequest("echo hello > file.txt", "../outside"))
  ...
  assert.match(decision.metadata?.executeCommandPolicy?.reason ?? "", /outside the workspace/i)
  assert.match(decision.metadata?.executeCommandPolicy?.reason ?? "", /\.\.\/outside/)
})

test("outside-workspace cwd does not override denied commands", async () => {
  const provider = createExecuteCommandGuardrailProvider({
    classifier: {
      classify() {
        return buildPolicy({
          ...
          disposition: "deny",
          profile: "host_unsafe",
          reason: "Dangerous host command is denied.",
          summary: "Dangerous host command denied (rm)."
        })
      }
    },
    predictor: {
      async predictExecute() {
        throw new Error("predictor should not run for denied commands")
      }
    }
  })

  const decision = await provider.evaluate(buildRequest("rm -rf /", "../outside"))

  assert.equal(decision.allow, false)
  assert.equal(decision.metadata?.executeCommandPolicy?.disposition, "deny")
  assert.match(decision.reasons?.[0]?.message ?? "", /dangerous host command/i)
})

test("execute cwd must be a string", async () => {
  const provider = createExecuteCommandGuardrailProvider({
    classifier: {
      classify() {
        throw new Error("classifier should not run for invalid cwd")
      }
    },
    predictor: {
      async predictExecute() {
        throw new Error("predictor should not run for invalid cwd")
      }
    }
  })

  const decision = await provider.evaluate(buildRequest("echo hello > file.txt", 42))

  assert.equal(decision.allow, false)
  assert.match(decision.reasons?.[0]?.message ?? "", /cwd must be a string/i)
})
...
test("predictable mutations are denied when target files cannot be predicted", async () => {
  ...
  assert.equal(decision.allow, false)
  assert.equal(decision.reasons?.[0]?.code, "jingle.controlled_shell_denied")
  assert.equal(decision.metadata?.executeCommandPolicy?.profile, "predictable_mutation")
