...
test("runtime reconciler preserves JSX accessory arrays separately from plain accessory arrays", async () => {
  ...
  assert.equal(
    ...
    "jingle-extension-asset://runtime-fixture/assets/notion-logo.png"
  )
  assert.equal(
    resolveRuntimeVisualImageSource({
      extensionName: snapshot.extensionName,
    ...
    "jingle-extension-asset://runtime-fixture/assets/icon/view_list.png"
  )
  assert.equal(
    resolveRuntimeVisualImageSource({
      extensionName: snapshot.extensionName,
...
test("runtime Action.RunBotAgent delegates to the agent host capability", async () => {
  const hostRequests: ExtensionHostRequest[] = []
  const renderer = createTestRenderer({
    onHostRequest: (request) => {
      hostRequests.push(request)
      ...
      createElement(List.Item, {
        actions: createElement(
          ...
          createElement(Action.RunBotAgent, {
            input: {
              prompt: {
                instructions: ["Inspect the issue first."],
                objective: "Fix the GitHub issue"
              },
              sourceRef: {
                id: "JunJD/Jingle#12",
                label: "GitHub issue JunJD/Jingle#12",
                type: "github.issue",
