...
const realRaycastExtensionsRepo =
  process.env.JINGLE_RAYCAST_EXTENSIONS_REPO ??
...
test("Raycast migration rewrites runtime imports through static module specifiers", () => {
  ...
  assert.match(rewritten, /import type \{ LaunchProps \} from "@jingle\/extension-api"/)
  assert.match(rewritten, /import \{ showToast \} from "@jingle\/extension-api"/)
  assert.match(rewritten, /export \{ Action \} from "@jingle\/extension-api"/)
  assert.match(rewritten, /import\("@jingle\/extension-api"\)/)
  assert.match(rewritten, /import\(`@jingle\/extension-utils`\)/)
...
test("Raycast migration gates known extension transforms by target context", () => {
  const source = [
    ...
    'import { OAuthService } from "@jingle/extension-utils"',
  ...
  const githubResult = rewriteSourceForJingle(
    source,
    "src/oauth.ts",
    { extensionId: "github", sourceExtensionId: "github" },
    { knownTransforms: transforms }
  )
  const notionResult = rewriteSourceForJingle(
    source,
    "src/oauth.ts",
    { extensionId: "notion", sourceExtensionId: "notion" },
    { knownTransforms: transforms }
...
test("Raycast migration shell host entry keeps migrated source out of compiled host entrypoints", async () => {
  ...
  try {
    ...
    const manifestSource = String(artifacts["jingle-package/manifest.ts"])
    const runtimeSource = String(artifacts["jingle-package/runtime.ts"])
    const toolsSource = String(artifacts["jingle-package/main/tools.ts"])
    const typecheckConfig = JSON.parse(String(artifacts["jingle-package/tsconfig.check.json"]))
    ...
    assert.equal(artifacts["jingle-package/src/tools/create-issue.ts"], undefined)
    assert.match(
      String(artifacts["jingle-package/src/create-issue.tsx"]),
      /not wired into the Jingle runtime/
    )
    assert.equal(artifacts["jingle-package/src/my-issues.meta.ts"] !== undefined, true)
    assert.equal(Buffer.isBuffer(artifacts["jingle-package/assets/github.png"]), true)
  } finally {
...
test("Raycast migration shell host entry keeps no-view commands in runtime metadata", async () => {
  ...
  try {
    ...
    const artifacts = buildRaycastAiMigrationArtifacts(preview)
    const manifestSource = String(artifacts["jingle-package/manifest.ts"])
    const runtimeMetadataSource = String(artifacts["jingle-package/runtime-metadata.ts"])
    const runtimeSource = String(artifacts["jingle-package/runtime.ts"])
    ...
    assert.match(String(artifacts["jingle-package/src/quick-add-reminder.tsx"]), /not wired/)
  } finally {
    await rm(extensionRoot, { force: true, recursive: true })
  }
})
...
test("Raycast migration migrated-source host entry wires no-view command source into runtime", async () => {
  ...
  try {
    ...
    const runtimeMetadataSource = String(artifacts["jingle-package/runtime-metadata.ts"])
    const runtimeSource = String(artifacts["jingle-package/runtime.ts"])
    const quickAddSource = String(artifacts["jingle-package/src/quick-add-reminder.tsx"])
    ...
    assert.doesNotMatch(runtimeSource, /run: async \(\) => \{\}/)
    assert.match(quickAddSource, /from "@jingle\/extension-api"/)
...
test("Raycast migration preview reports dependency decisions and unsupported APIs", async () => {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-preview-"))
  ...
  try {
    ...
    await writeFile(
      ...
      [
        ...
        "  return (",
        "    <Form>",
        '      <Form.Description text="Supported in Raycast" />',
    ...
    assert.deepEqual(Object.keys(buildRaycastAiMigrationArtifacts(packagePreview)).sort(), [
      ...
      "jingle-package/assets/fixture.png",
      "jingle-package/identity.ts",
      "jingle-package/main.ts",
      "jingle-package/main/migrated-src/oauth.ts",
      "jingle-package/main/migrated-src/tools/search-pages.ts",
      "jingle-package/main/migrated-src/utils/notion.ts",
      "jingle-package/main/tools.ts",
      "jingle-package/manifest.ts",
      "jingle-package/package.json",
      "jingle-package/runtime-metadata.ts",
      "jingle-package/runtime.ts",
      "jingle-package/src/index.tsx",
      "jingle-package/src/oauth.ts",
      "jingle-package/src/search-page.meta.ts",
      "jingle-package/src/search-page.tsx",
      "jingle-package/src/tools/search-pages.ts",
      "jingle-package/src/utils/notion.ts",
      "jingle-package/tsconfig.check.json",
      "jingle-package/types.d.ts",
      "main.preview.ts",
      "manifest.patch.json",
      "manifest.preview.ts",
    ...
    assert.deepEqual(pickDependency(preview, "@raycast/api"), {
      ...
      name: "@raycast/api",
      jingleTarget: "@jingle/extension-api",
      version: "^1.104.5"
    })
    ...
    assert.deepEqual(packagePreviewJson, {
      dependencies: {
        ...
        "@jingle/extension-utils": "workspace:*",
        "date-fns": "^4.3.0",
        linkedom: "^0.18.12",
        "notion-to-md": "^3.1.9",
        zod: "^4.0.0"
    ...
    const jinglePackageJson = JSON.parse(
      await readFile(join(artifactDir, "jingle-package", "package.json"), "utf8")
    )
    assert.deepEqual(jinglePackageJson, packagePreviewJson)
    const jingleMain = await readFile(join(artifactDir, "jingle-package", "main.ts"), "utf8")
    assert.match(jingleMain, /from "\.\/main\/tools"/)
    assert.match(jingleMain, /fixtureMain/)
    assert.match(jingleMain, /createFixtureTools/)
    assert.doesNotMatch(jingleMain, /@raycast\/api|@raycast\/utils/)
    const jingleManifest = await readFile(
      join(artifactDir, "jingle-package", "manifest.ts"),
      "utf8"
    )
    assert.match(jingleManifest, /defineNativeExtensionManifest/)
    assert.match(jingleManifest, /fixtureManifest/)
    assert.match(
      jingleManifest,
    ...
    assert.match(jingleTools, /createFixtureTools/)
    assert.match(jingleTools, /runMigratedTool/)
    assert.match(jingleTools, /requestMigratedToolHost/)
    assert.match(jingleTools, /request\.capability === "toast"/)
    assert.match(jingleTools, /request\.capability === "navigation"/)
    assert.match(jingleTools, /searchPagesConfirmation/)
    assert.match(jingleTools, /approval: \{/)
    assert.match(jingleTools, /import\("\.\.\/main\/migrated-src\/tools\/search-pages"\)\)\.default/)
    assert.match(jingleTools, /import\("\.\.\/main\/migrated-src\/tools\/search-pages"\)\)\.confirmation/)
    assert.doesNotMatch(jingleTools, /createMigratedTools/)
    assert.doesNotMatch(
      jingleTools,
      /Migrate src\/tools\/search-pages\.ts into a Jingle handler/
    )
    assert.doesNotMatch(jingleTools, /@raycast\/api|@raycast\/utils/)

    const jingleRuntime = await readFile(
      join(artifactDir, "jingle-package", "runtime.ts"),
      "utf8"
    )
    assert.match(jingleRuntime, /defineNativeExtensionRuntime/)
    assert.match(jingleRuntime, /FixtureSearchPageCommand/)
    assert.match(jingleRuntime, /import FixtureSearchPageCommandSource from "\.\/src\/search-page"/)
    assert.match(jingleRuntime, /FixtureSearchPageCommandSource as ComponentType/)
    assert.match(jingleRuntime, /extensionName: "fixture"/)
    assert.match(jingleRuntime, /"\bsearch-page\b"/)
    assert.doesNotMatch(jingleRuntime, /@raycast\/api|@raycast\/utils/)

    const generatedCommandMeta = await readFile(
      join(artifactDir, "jingle-package", "src", "search-page.meta.ts"),
      "utf8"
    )
    assert.match(generatedCommandMeta, /bodyHeight: 520/)

    const generatedCommandWrapper = await readFile(
      join(artifactDir, "jingle-package", "src", "search-page.tsx"),
      "utf8"
    )
    assert.equal(generatedCommandWrapper, 'export { default } from "./index"\n')

    const jingleRuntimeMetadata = await readFile(
      join(artifactDir, "jingle-package", "runtime-metadata.ts"),
      "utf8"
    )
    assert.match(jingleRuntimeMetadata, /defineNativeExtensionRuntimeMetadata/)
    assert.match(jingleRuntimeMetadata, /"name": "search-page"/)
    assert.match(jingleRuntimeMetadata, /aliases/)
    assert.match(jingleRuntimeMetadata, /keywords/)
    const jingleIdentity = await readFile(
      join(artifactDir, "jingle-package", "identity.ts"),
      "utf8"
    )
    assert.match(jingleIdentity, /subjectTerms: \[\s*"fixture"\s*\]/)
    assert.match(jingleIdentity, /export const EXTENSION_SUBJECT_TERMS = EXTENSION_IDENTITY\.subjectTerms/)
    assert.doesNotMatch(jingleRuntimeMetadata, /resolveCommand|buildIntentItems/)
    assert.doesNotMatch(jingleRuntimeMetadata, /config\.aliases\.includes\(query\)/)
    assert.doesNotMatch(jingleRuntimeMetadata, /@shared\/launcher/)

    const typecheckConfig = JSON.parse(
      await readFile(join(artifactDir, "jingle-package", "tsconfig.check.json"), "utf8")
    )
    assert.deepEqual(typecheckConfig.compilerOptions.paths["@jingle/extension-api"], [
      join(repoRoot, "packages/extension-api/src/index.ts")
    ])
    assert.deepEqual(typecheckConfig.compilerOptions.paths["@jingle/extension-api/host-runtime"], [
      join(repoRoot, "packages/extension-api/src/host-runtime.ts")
    ])
    assert.deepEqual(typecheckConfig.compilerOptions.paths["@jingle/extension-utils"], [
    ...
    const jingleTypes = await readFile(
      join(artifactDir, "jingle-package", "types.d.ts"),
      "utf8"
    )
    assert.match(jingleTypes, /namespace Preferences/)
    assert.match(jingleTypes, /type Extension/)
    assert.match(jingleTypes, /open_in\?: RuntimeOpenApplication/)
    assert.match(jingleTypes, /namespace Arguments/)
    assert.match(jingleTypes, /type SearchPage/)
    assert.match(jingleTypes, /query\?: string/)

    const migratedCommandSource = await readFile(
      join(artifactDir, "jingle-package", "src", "index.tsx"),
      "utf8"
    )
    assert.match(migratedCommandSource, /from "@jingle\/extension-api"/)
    ...
    const migratedOauthSource = await readFile(
      join(artifactDir, "jingle-package", "src", "oauth.ts"),
      "utf8"
    )
    assert.doesNotMatch(migratedOauthSource, /raycast\.com/)

    const migratedToolSource = await readFile(
      join(artifactDir, "jingle-package", "src", "tools", "search-pages.ts"),
      "utf8"
    )
    assert.match(migratedToolSource, /from "@jingle\/extension-api"/)
    assert.match(migratedToolSource, /from "@jingle\/extension-utils"/)
    assert.doesNotMatch(migratedToolSource, /@raycast\/api|@raycast\/utils/)

    const migratedAsset = await readFile(
      join(artifactDir, "jingle-package", "assets", "fixture.png"),
      "utf8"
    )
    ...
    assert.deepEqual(
      toolsPreview.map((tool: { jingleName: string }) => tool.jingleName),
      ["searchPages"]
    )
    ...
    assert.match(
      toolsPreviewSource,
      /import type \{ ExtensionToolConfirmation, ExtensionToolContext, ExtensionToolDefinition \} from "@jingle\/extension-api"/
    )
    ...
    assert.match(
      toolsPreviewSource,
      /import\("\.\/jingle-package\/main\/migrated-src\/tools\/search-pages"\)\)\.default/
    )
    assert.match(
      toolsPreviewSource,
      /import\("\.\/jingle-package\/main\/migrated-src\/tools\/search-pages"\)\)\.confirmation/
    )
...
test("Raycast migration preview wires git extension command sources into generated runtime", async () => {
  const repoRootDir = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-git-"))
  ...
  try {
    ...
    execFileSync(
      ...
      [
        ...
        repoRootDir,
        "-c",
        "user.name=Jingle Test",
        "-c",
        "user.email=jingle@example.test",
        "commit",
        "-m",
        "fixture"
      ],
    ...
    assert.match(
      String(artifacts["jingle-package/runtime.ts"]),
    ...
    assert.ok(artifacts["jingle-package/src/search-page.tsx"])
  } finally {
...
test("Raycast migration preview infers host capabilities from action components", async () => {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-actions-"))
...
test("Raycast migration preview infers host capabilities from navigation and HUD helpers", async () => {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-helpers-"))
...
test("Raycast migration preview supports Raycast preferences settings helpers", async () => {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-preferences-"))
  ...
  try {
    ...
    assert.equal(preview.unsupportedApis.counts.blockingIssues, 0)
    assert.match(
      String(artifacts["jingle-package/src/command.tsx"]),
      /from "@jingle\/extension-api"/
    )
    assert.match(String(artifacts["jingle-package/src/command.tsx"]), /openNativeCommandSettings/)
    assert.match(String(artifacts["jingle-package/src/command.tsx"]), /openNativeExtensionSettings/)
    assert.doesNotMatch(
      String(artifacts["jingle-package/src/command.tsx"]),
...
test("Raycast migration preview infers storage capability from storeValue controls", async () => {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-store-value-"))
...
test("Raycast migration preview does not infer runtime capabilities from type-only Raycast imports", async () => {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-type-only-"))
...
test("Raycast migration generated tool handlers run migrated Raycast auth utilities", async () => {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-tools-"))
  const artifactDir = join(extensionRoot, "migration-artifacts")
  ...
  try {
    ...
    const artifacts = buildRaycastAiMigrationArtifacts(preview)
    const migratedOauthSource = String(artifacts["jingle-package/src/oauth.ts"])
    assert.doesNotMatch(migratedOauthSource, /getConnectionSecret/)

    await writeArtifacts(artifactDir, artifacts)
    await writeFile(
      join(artifactDir, "jingle-package", "run-tool.ts"),
      [
        'import assert from "node:assert/strict"',
...
test("Raycast migration generated Notion-style tools initialize module Notion client inside SDK context", async () => {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-notion-tool-"))
  const artifactDir = join(extensionRoot, "migration-artifacts")
  ...
  try {
    ...
    await writeArtifacts(artifactDir, artifacts)
    await writeFile(
      join(artifactDir, "jingle-package", "run-notion-tool.ts"),
      [
        'import assert from "node:assert/strict"',
        'import { createFixtureTools } from "./main/tools"',
        "",
...
test("Raycast migration treats no-input tools as fully schematized", async () => {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-empty-tool-"))
...
test("Raycast migration rewrites Notion form property values to current API wrappers", async () => {
  const extensionRoot = await mkdtemp(
    join(tmpdir(), "jingle-raycast-migration-notion-properties-")
  )
  ...
  try {
    ...
    const migratedPropertySource = String(
      buildRaycastAiMigrationArtifacts(preview)[
        "jingle-package/src/utils/notion/page/property.ts"
      ]
    )
...
test("Raycast migration generated UI command uses Jingle runtime facade host requests", async () => {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-ui-command-"))
  const artifactDir = join(extensionRoot, "migration-artifacts")
  ...
  try {
    ...
    await writeFile(
      join(artifactDir, "jingle-package", "runtime-ui-command-check.ts"),
      [
        'import assert from "node:assert/strict"',
...
test("Raycast migration generated package passes native extension registry validation", async () => {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-registry-"))
  const artifactDir = join(extensionRoot, "migration-artifacts")
  ...
  try {
    ...
    const artifacts = buildRaycastAiMigrationArtifacts(preview)
    const packageJson = JSON.parse(String(artifacts["jingle-package/package.json"]))
    ...
    assert.equal(packageJson.name, "@jingle/extension-fixture-generated")
    assert.match(
      String(artifacts["jingle-package/identity.ts"]),
    ...
    assert.match(
      String(artifacts["jingle-package/manifest.ts"]),
      /"id": EXTENSION_ID/
    )
    assert.match(
      String(artifacts["jingle-package/runtime-metadata.ts"]),
      /extensionName: EXTENSION_ID/
    )
    assert.match(
      String(artifacts["jingle-package/src/search-page.tsx"]),
      /jingle:\/\/extensions\/fixture-generated\/search-page\?launchContext=/
    )
    assert.doesNotMatch(
      String(artifacts["jingle-package/src/search-page.tsx"]),
    ...
    await writeFile(
      join(artifactDir, "jingle-package", "registry-check.ts"),
      [
        'import assert from "node:assert/strict"',
...
test("Raycast migration preview normalizes scoped package names into valid Jingle extension ids", async () => {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-preview-scoped-"))
...
test("Raycast migration preview rejects explicit invalid target extension ids", async () => {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-preview-invalid-id-"))
...
test("Raycast migration generated Notion-style package keeps runtime contracts reproducible", async () => {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-notion-contract-"))
  const artifactDir = join(extensionRoot, "migration-artifacts")
  ...
  try {
    ...
    const migratedPropertySource = String(
      artifacts["jingle-package/src/utils/notion/page/property.ts"]
    )
    assert.match(migratedPropertySource, /return \{ title: markdownToRichText\(value\) \}/)
    assert.match(migratedPropertySource, /return \{ rich_text: markdownToRichText\(value\) \}/)
    assert.match(migratedPropertySource, /return \{ number: parseFloat\(value\) \}/)
    ...
    assert.equal(artifacts["jingle-package/assets/.gitkeep"], "")
    ...
    await writeFile(
      join(artifactDir, "jingle-package", "runtime-contract-check.ts"),
      [
        'import assert from "node:assert/strict"',
        'import { fixtureGeneratedRuntime } from "./runtime"',
        'import { fixtureGeneratedRuntimeMetadata } from "./runtime-metadata"',
...
test("Raycast migration generated runtime supports top-level preference reads in command sources", async () => {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-runtime-static-"))
  const artifactDir = join(extensionRoot, "migration-artifacts")
  ...
  try {
    ...
    await writeFile(
      join(artifactDir, "jingle-package", "runtime-static-check.ts"),
      [
        'import assert from "node:assert/strict"',
        'import { createElement } from "react"',
        'import { fixtureRuntime } from "./runtime"',
...
async function createGithubLikeExtensionFixture() {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-github-like-"))
  await mkdir(join(extensionRoot, "assets"), { recursive: true })
  await mkdir(join(extensionRoot, "src", "tools"), { recursive: true })
...
async function createAppleRemindersLikeExtensionFixture() {
  const extensionRoot = await mkdtemp(join(tmpdir(), "jingle-raycast-migration-apple-like-"))
  await mkdir(join(extensionRoot, "assets"), { recursive: true })
  await mkdir(join(extensionRoot, "src", "tools"), { recursive: true })
...
function assertGeneratedPackageContract(
  ...
  const packageJson = JSON.parse(String(artifacts["jingle-package/package.json"]))
  assert.equal(packageJson.name, `@jingle/extension-${extensionId}`)
  assert.equal(packageJson.dependencies["@jingle/extension-api"], "workspace:*")
  assert.equal(packageJson.dependencies["@jingle/extension-utils"], "workspace:*")
}
...
function assertGeneratedSourcesDoNotImportRaycastRuntime(
  ...
  for (const [file, content] of Object.entries(artifacts)) {
    if (!file.startsWith("jingle-package/") || !/\.[cm]?tsx?$/.test(file)) {
      continue
    }
...
function pickDependency(
  ...
  return {
    ...
    jingleTarget: dependency.jingleTarget,
