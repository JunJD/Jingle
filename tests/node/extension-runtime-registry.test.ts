...
test("Figma Files keeps the package contract", async () => {
  ...
  assert.equal(figmaFilesPackage.name, "@jingle/extension-figma-files")
  assert.deepEqual(figmaFilesPackage.jingle, {
...
test("Notion keeps the migrated manifest and package contract", async () => {
  ...
  assert.equal(notionPackage.name, "@jingle/extension-notion")
  assert.deepEqual(notionPackage.jingle, {
    distribution: "installable",
    trust: "trusted"
  })
...
test("declared extension icon assets exist inside their extension packages", () => {
  ...
  assert.equal(
    ...
    "jingle-extension-asset://github/assets/icon.svg"
  )
  assert.equal(
    createNativeExtensionAssetUrl({
      extensionName: "notion",
    ...
    "jingle-extension-asset://notion/assets/notion-logo.png"
  )
})
