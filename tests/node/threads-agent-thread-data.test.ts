...
function createMemoryContextSnapshot(): JingleMemoryContextSnapshot {
...
test("threads service restores provided context inclusions from frozen run metadata when checkpoint has no context state", async () => {
  ...
  const service = createService({
    latestRunMetadata: {
      [JINGLE_MEMORY_CONTEXT_SNAPSHOT_METADATA_KEY]: memoryContextSnapshot
    },
    messages: [],
    todos: []
  })
...
test("agent thread data snapshot service composes persisted and live snapshots", async () => {
  ...
  assert.equal(snapshot.thread.status, "busy")
  assert.deepEqual(
...
test("agent thread data snapshot service returns persisted snapshot without live runtime", async () => {
  ...
  const service = new AgentThreadDataSnapshotService(
    ...
    {
      readLiveThreadDataSnapshot: () => null
    }
  )
