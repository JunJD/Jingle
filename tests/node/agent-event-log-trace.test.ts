...
async function createHarnessTraceCallback(input: {
  modelId: string
  runId: string
  threadId: string
}) {
  const { appendAgentEventSafely } = await loadDbModules()
  return createJingleLangChainTraceCallback({
...
test.before(async () => {
  jingleHome = await mkdtemp(join(tmpdir(), "jingle-agent-event-log-trace-"))
...
test("harness trace callback captures chat model input summary without full messages", async () => {
  const {
    createRun,
    createThread,
    getAgentTraceEvents,
  ...
  const callback = await createHarnessTraceCallback({
    modelId: "gpt-test",
    runId,
    threadId
  })
  const llm = {
    id: ["jingle", "test-model"],
    kwargs: {},
    lc: 1,
    name: "test-model",
    type: "constructor"
...
test("harness trace callback does not capture title generation runs", async () => {
