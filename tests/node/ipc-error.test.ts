...
import { normalizeInvokeError, OpenworkIpcClientError } from "../../src/preload/ipc-errors"
import {
...
test("buildIpcErrorPayload maps validation-style failures to invalid argument", () => {
  const payload = buildIpcErrorPayload(
    ...
    new OpenworkIpcError({
      ...
      details: ["command.resume.request_id: required"],
      message: "agent:resume params validation failed. command.resume.request_id: required"
    })
  )
...
test("normalizeInvokeError rehydrates preload errors into client errors", () => {
  ...
  assert.ok(error instanceof OpenworkIpcClientError)
...
test("extractIpcErrorPayload falls back to the name field when code metadata is already decoded", () => {
...
test("normalizeInvokeError rehydrates decoded errors even when channel metadata is missing", () => {
  ...
  assert.ok(normalized instanceof OpenworkIpcClientError)
