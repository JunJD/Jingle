...
test("search_history tool content includes retrieved message bodies for the model", () => {
  ...
  const content = contextRetrievalToolHandlersInternals.formatRetrievedHistoryToolContent({
    digests: [],
    messages: [
      {
        message,
...
test("search_history schema accepts the documented maximum limit", () => {
  const parsed = contextRetrievalToolHandlersInternals.searchHistorySchema.parse({
    limit: 20,
    query: "生成",
    threadId: "thread-1"
  })
...
test("get_message_context tool content includes a bounded transcript window", () => {
  ...
  const content = contextRetrievalToolHandlersInternals.formatRetrievedMessageToolContent({
    after: 1,
    before: 1,
...
test("getProjectedMessageWindow returns the focus message with requested neighbors", () => {
  ...
  assert.deepEqual(
    contextRetrievalToolHandlersInternals
  ...
  assert.deepEqual(
    contextRetrievalToolHandlersInternals.getProjectedMessageWindow({
      after: 2,
      before: 2,
...
test("get_trace_evidence tool content includes bounded trace blobs", () => {
  ...
  const content = contextRetrievalToolHandlersInternals.formatRetrievedTraceEvidenceToolContent({
