...
import { JINGLE_TOOL_EXECUTION_METADATA_KEY } from "@jingle/agent-client"
import { projectJingleAgentActivitySummary } from "@jingle/agent-react"
...
function createSteeringUserMessage(
  id: string,
  ...
  return {
    ...
    metadata: mergeJingleSteeringStatusMetadata(undefined, status)
  }
}

function createSteeringAppliedMarkerMessage(id: string, userMessageId: string): Message {
  return {
    content: "",
...
test("agent activity summary groups known tool categories without using tool names", () => {
  ...
  assert.deepEqual(
    projectJingleAgentActivitySummary([
      ...
      {
        status: "running",
        toolCall: createToolCall("tool-web", "web_search", { query: "Jingle runtime" })
      }
    ]),
    {
...
test("unchanged snapshots reuse the previous projection object", () => {
  ...
  const firstProjection = projectMessages(messages)
  const stableMessages = stabilizeJingleMessageList(messages, cloneMessages(messages))
  const nextProjection = projectMessages(stableMessages, firstProjection)
...
test("pending steering user messages do not steal in-flight tool results from the active turn", () => {
  const toolCall = createToolCall("tool-call-1")
  const projection = projectMessages([
    createUserMessage("user-1", "Run tool"),
    createAssistantMessage({ id: "assistant-1", toolCalls: [toolCall] }),
    createSteeringUserMessage("steer-1", "pending", "Use the smaller fix"),
    createToolMessage({
      content: "tool result",
      id: "tool-result-1",
      toolCallId: toolCall.id
...
test("applied steering markers stay out of display rows while the steer turn remains visible", () => {
  const toolCall = createToolCall("tool-call-1")
  const projection = projectMessages([
    createUserMessage("user-1", "Run tool"),
    createAssistantMessage({ id: "assistant-1", toolCalls: [toolCall] }),
    createSteeringUserMessage("steer-1", "applied", "Use the smaller fix"),
    createToolMessage({
      content: "tool result",
      id: "tool-result-1",
      toolCallId: toolCall.id
    }),
    createSteeringAppliedMarkerMessage("steer-applied:steer-1", "steer-1"),
    createAssistantMessage({ content: "Guided answer", id: "assistant-guided-1" })
  ])
  ...
  assert.equal(projection.turns[1]?.key, "steer-1")
  assert.deepEqual(
    projection.turns[1]?.assistants.map((message) => message.id),
    ["assistant-guided-1"]
  )
  assert.deepEqual(
    projection.displayRows.map((row) => row.kind),
...
test("prepend and append preserve unchanged turn references by message id", () => {
  ...
  const prependedMessages = stabilizeJingleMessageList(currentMessages, prependedSnapshot)
  ...
  const appendedMessages = stabilizeJingleMessageList(prependedMessages, appendedSnapshot)
