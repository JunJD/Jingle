...
function createAgentState(
  ...
  stateSchema?: any,
  middlewareList: readonly any[] = []
) {
  /**
   * Collect fields from state schemas
   */
  const stateFields: Record<string, any> = { jumpTo: new UntrackedValue() }
        ...
        if (isZodSchemaV4(fieldSchema)) {
          ...
          if (meta?.reducer) {
            if (meta.reducer.schema) {
              const ReducedValueConstructor = ReducedValue as any
              stateFields[key] = new ReducedValueConstructor(fieldSchema, {
  ...
  for (const middleware of middlewareList)
    if (
      ...
      (StateSchema.isInstance(middleware.stateSchema) || isInteropZodObject(middleware.stateSchema))
    )
