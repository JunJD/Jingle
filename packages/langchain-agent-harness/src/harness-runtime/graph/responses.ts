...
class ToolStrategy {
  ...
  constructor(schema: any, tool: any, options?: any) {
    this.schema = schema
    this.tool = tool
  ...
  static fromSchema(schema: any, outputOptions?: any) {
    /**
...
class ProviderStrategy {
  ...
  constructor(schemaOrOptions: any, strict?: boolean) {
    if (
  ...
  static fromSchema(schema: any, strict?: boolean) {
 ...
 * @returns
 */
function transformResponseFormat(responseFormat: any, options?: any, model?: any): any[] {
  ...
  if (Array.isArray(responseFormat)) {
    ...
    if (
      responseFormat.every(
        (item) => item instanceof ToolStrategy || item instanceof ProviderStrategy
      )
    )
    ...
    if (responseFormat.every((item) => isSerializableSchema(item)))
      return responseFormat.map((item) => ToolStrategy.fromSchema(item, options))
    /**
 ...
 * ```
 */
function toolStrategy(responseFormat: any, options?: any) {
  return transformResponseFormat(responseFormat, options)
}
function providerStrategy(responseFormat: any) {
  /**
 ...
 */
function hasSupportForJsonSchemaOutput(model: any) {
  if (!model) return false
  if (typeof model === "string") {
    const modelName = model.split(":").pop() ?? ""
    return MODEL_NAMES_THAT_SUPPORT_JSON_SCHEMA_OUTPUT.some((modelNameSnippet) =>
      modelName.includes(modelNameSnippet)
    )
  }
  ...
  if (
    ...
    (("model" in model &&
      MODEL_NAMES_THAT_SUPPORT_JSON_SCHEMA_OUTPUT.some(
        (modelNameSnippet) =>
          typeof model.model === "string" && model.model.includes(modelNameSnippet)
      )) ||
      (chatModelClass === "FakeToolCallingModel" && "structuredResponse" in model))
  )
    return true
  return false
}
