...
import { mergeAbortSignals } from "./execution-kernel-utils.js"
 ...
 */
const getInvalidToolError = (toolName, availableTools) =>
...
/**
 * Internal LangGraph-backed tool execution kernel used by ToolStepNode.
 */
class RuntimeToolExecutionKernel extends RunnableCallable {
  options: any
  tools: any[]
  trace = false
  signal?: AbortSignal
  handleToolErrors: any = defaultHandleToolErrors
  wrapToolCall: any

  constructor(tools: any[], options?: any) {
   ...
   */
  #handleError(error: any, call: any, isMiddlewareError: boolean) {
    /**
  ...
  async runTool(call: any, config: any, state: any) {
    /**
  ...
  async run(state: any, config: any) {
    ...
    if (isSendInput(state)) {
      ...
      if (isBaseMessageArray(state)) messages = state
        ...
        throw new Error(
          "RuntimeToolExecutionKernel only accepts BaseMessage[] or { messages: BaseMessage[] } as input."
        )
      const toolMessageIds = new Set(
        messages.filter((msg) => msg.getType() === "tool").map((msg) => msg.tool_call_id)
      )
      let aiMessage
      ...
      if (!AIMessage.isInstance(aiMessage))
        throw new Error("RuntimeToolExecutionKernel only accepts AIMessages as input.")
...
function isSend(x: any) {
  return x instanceof Send
}
export { RuntimeToolExecutionKernel }
