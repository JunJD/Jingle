...
import { mergeAbortSignals, toPartialZodObject } from "./execution-kernel-utils.js"
...
/**
 * The legacy LangGraph destination name used by LangChain model-call commands.
 */
const MODEL_REQUEST_NODE_NAME = "model_request"
class RuntimeModelExecutionKernel extends RunnableCallable {
  #options: any
  #systemMessage: any
   ...
   */
  #getResponseFormat(model: any): any {
     ...
     */
    if (!strategies.every((format) => format instanceof ProviderStrategy))
      return {
        type: "tool",
        tools: strategies
  ...
  async #run(state: any, config: any) {
    /**
    ...
    if (
      ...
      const { structuredResponse, messages } = response as any
      return {
  ...
  async #invokeModel(state: any, config: any, options: any = {}) {
     ...
     */
    const baseHandler = async (request: any) => {
      /**
      ...
      const response: any = await raceWithSignal(
      ...
      const toolMessageContent =
        structuredResponseFormat.tools[toolCalls[0].name]?.options?.toolMessageContent
    ...
    const wrapperMiddleware = this.#options.modelCallWrappers ?? []
      ...
      if (middleware.wrapModelCall) {
        ...
        wrappedHandler = async (request) => {
          ...
          const context = currentMiddleware.contextSchema
            ? interopParse(currentMiddleware.contextSchema, lgConfig?.context || {})
            : lgConfig?.context
          /**
           * Create runtime
           */
          ...
          const requestWithStateAndRuntime = {
            ...
            state: {
              ...(middleware.stateSchema
                ? interopParse(toPartialZodObject(middleware.stateSchema), state)
                : {}),
              ...currentGetState(),
          ...
          const handlerWithValidation = async (req: any) => {
             ...
             * We can't allow this as the runtime tool execution kernel is already initiated with given tools.
             */
            const modifiedTools = req.tools ?? []
            const newTools = modifiedTools.filter(
              (tool) =>
                isClientTool(tool) && !this.#options.toolClasses.some((t) => t.name === tool.name)
            )
            if (newTools.length > 0)
              throw new Error(
                `You have added a new tool in "wrapModelCall" hook of middleware "${currentMiddleware.name}": ${newTools.map((tool) => tool.name).join(", ")}. This is not supported.`
              )
            /**
            ...
            const invalidTools = modifiedTools.filter(
              (tool) => isClientTool(tool) && this.#options.toolClasses.every((t) => t !== tool)
            )
            if (invalidTools.length > 0)
              throw new Error(
                `You have modified a tool in "wrapModelCall" hook of middleware "${currentMiddleware.name}": ${invalidTools.map((tool) => tool.name).join(", ")}. This is not supported.`
              )
          ...
          try {
            ...
            if (!isInternalModelResponse(middlewareResponse))
              throw new Error(
                `Invalid response from "wrapModelCall" in middleware "${currentMiddleware.name}": expected AIMessage or Command, got ${typeof middlewareResponse}`
              )
   ...
   */
  #handleMultipleStructuredOutputs(response: any, toolCalls: any[], responseFormat: any) {
   ...
   */
  #handleSingleStructuredOutput(response: any, toolCall: any, responseFormat: any, lastMessage?: any) {
  ...
  async #handleToolStrategyError(error: any, response: any, toolCall: any, responseFormat: any) {
    /**
     ...
     */
    const firstToolStrategy = Object.values(responseFormat.tools).at(0) as any
    const errorHandler = firstToolStrategy?.options?.handleError
  ...
  #areMoreStepsNeeded(state: any, response: any) {
  ...
  async #bindTools(model: any, preparedOptions: any, structuredResponseFormat: any) {
    ...
    if (structuredResponseFormat?.type === "native") {
      const resolvedStrict =
        preparedOptions?.modelSettings?.strict ?? structuredResponseFormat?.strategy?.strict ?? true
    ...
    return this.#options.includeAgentName === "inline"
...
export { MODEL_REQUEST_NODE_NAME, RuntimeModelExecutionKernel }
