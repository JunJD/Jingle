...
const defaultOutputRoot = resolve(repoRoot, ".ow-build", "installed-extensions")
const reactBridgeGlobalKey = "openwork.extensionRuntime.reactBridge"
...
if (command === "build") {
  ...
  if (extensionRefs.length === 0) {
    throw new Error("Usage: openwork-extension build <extension-id-or-path...> [--trust <level>]")
  }
  ...
  if (!extensionRef) {
    throw new Error("Usage: openwork-extension dev <extension-id-or-path> [--trust <level>]")
  }
  ...
  console.log(
    "Openwork dev discovers this root at process startup when ELECTRON_RENDERER_URL is set."
  )
  console.log(
    "Restart the Openwork dev app after rebuilds; extension hot reload is not implemented."
  )
...
Usage:
  openwork-extension build <extension-id-or-path...> [--out-dir <dir>] [--trust trusted|untrusted]
  openwork-extension dev <extension-id-or-path> [--out-dir <dir>] [--trust trusted|untrusted]
...
Trust:
  Package defaults to openwork.trust from package.json, or "untrusted" when omitted.
...
async function buildExtension(input) {
  ...
  writeJson(join(packageRoot, "openwork.extension.json"), {
    assets: "./assets",
    id: manifest.name,
    main: "./dist/main.mjs",
...
function resolvePackageTrust(packageJson, trustOverride) {
  ...
  const packageTrust = packageJson.openwork?.trust
  if (!packageTrust) {
    return "untrusted"
  }
  ...
  throw new Error('package.json openwork.trust must be "trusted" or "untrusted"')
}
...
async function loadBundledDefinitionModule(extensionRoot, entryName) {
  const cacheDir = join(repoRoot, ".ow-build", "extension-cli-cache")
  ...
  await build({
    ...
    packages: "bundle",
    platform: "node",
    plugins: [openworkExtensionApiAliasPlugin()],
    target: "node18"
  })
...
async function buildModuleFromSource(input) {
  const plugins = [openworkExtensionApiAliasPlugin()]
  if (input.installRuntimeReactShim) {
    plugins.push(openworkRuntimeShimPlugin())
  }

  await build({
    banner: {
      js: 'import { createRequire as __openworkCreateRequire } from "node:module"; const require = __openworkCreateRequire(import.meta.url);'
    },
    bundle: true,
...
function openworkExtensionApiAliasPlugin() {
  return {
    name: "openwork-extension-api-alias",
    setup(build) {
      build.onResolve({ filter: /^@openwork\/extension-api$/ }, () => ({
        path: resolve(repoRoot, "packages/extension-api/src/index.ts")
      }))
      build.onResolve({ filter: /^@openwork\/extension-api\/host-runtime$/ }, () => ({
        path: resolve(repoRoot, "packages/extension-api/src/host-runtime.ts")
      }))
      build.onResolve({ filter: /^@openwork\/extension-utils$/ }, () => ({
        path: resolve(repoRoot, "packages/extension-utils/src/index.ts")
      }))
    }
  }
}

function openworkRuntimeShimPlugin() {
  return {
    name: "openwork-runtime-shim",
    setup(build) {
      build.onResolve({ filter: /^react$/ }, () => ({
        namespace: "openwork-runtime-shim",
        path: "react"
      }))
      ...
      build.onLoad({ filter: /^react$/, namespace: "openwork-runtime-shim" }, () => ({
        contents: reactShimSource(),
        loader: "js"
      }))
      build.onLoad({ filter: /^react\/jsx-runtime$/, namespace: "openwork-runtime-shim" }, () => ({
        contents: reactJsxRuntimeShimSource("jsxRuntime"),
        loader: "js"
      }))
      build.onLoad(
        { filter: /^react\/jsx-dev-runtime$/, namespace: "openwork-runtime-shim" },
        () => ({
...
if (bridge.version !== ${reactBridgeVersion}) {
  throw new Error("Openwork extension runtime React bridge version " + bridge.version + " is not supported.")
}
const ${localName} = bridge.${bridgeProperty}
if (!${localName}) {
  throw new Error("Openwork extension runtime React bridge is missing ${bridgeProperty}.")
}
`
}
