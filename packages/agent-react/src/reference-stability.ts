...
export interface JingleStableMessageSource {
  id: string
}
...
export function stabilizeJingleReferences<T>(previous: T, next: T): T {
  return replaceEqualDeep(previous, next) as T
}

export function stabilizeJingleMessageList<TMessage extends JingleStableMessageSource>(
  previous: TMessage[],
  next: TMessage[]
): TMessage[] {
  ...
  const messages = next.map((nextMessage, index) => {
    ...
    const stableMessage = previousMessage
      ? stabilizeJingleReferences(previousMessage, nextMessage)
      : nextMessage
