...
function isReactComponentLike(value: unknown): boolean {
  ...
  const maybeComponent = value as { displayName?: string; name?: string }
  const name = maybeComponent.displayName ?? maybeComponent.name ?? ""
  return COMPONENT_NAME_PATTERN.test(name)
}

function resolveOpenworkAccessToken(
