...
const JINGLE_REPO_ROOT = resolve(dirname(fileURLToPath(import.meta.url)), "../../..")
...
const DEPENDENCY_DECISIONS = {
  "@mozilla/readability": {
    ...
    jingleTarget: "@mozilla/readability",
    reason:
  ...
  "@raycast/api": {
    ...
    reason: "Raycast runtime-bound UI and host APIs must map to the Jingle extension author API."
  },
  "@raycast/utils": {
    category: "runtime-facade",
    decision: "rewrite-import-with-adapters",
    jingleTarget: "@jingle/extension-utils",
    reason:
      "Hooks can map to Jingle utilities, but auth helpers such as withAccessToken need connection handling."
  },
  "@tryfabric/martian": {
    category: "direct-third-party",
    decision: "keep-direct-dependency",
    jingleTarget: "@tryfabric/martian",
    reason: "Markdown-to-Notion block conversion should be reused for migration fidelity."
  },
  "date-fns": {
    category: "direct-third-party",
    decision: "keep-direct-dependency",
    jingleTarget: "date-fns",
    reason: "Date formatting behavior can be preserved during migration."
  },
  linkedom: {
    category: "direct-third-party",
    decision: "keep-direct-dependency",
    jingleTarget: "linkedom",
    reason: "Readability needs an HTML parser for quick-capture webpage extraction."
  },
  "notion-to-md": {
    category: "direct-third-party",
    decision: "keep-direct-dependency",
    jingleTarget: "notion-to-md",
...
const RAYCAST_API_IMPORT_SUPPORT = {
  ...
  OAuth: {
    note: "Jingle preserves the Raycast OAuth shape for migrated code; Notion V1 resolves tokens from connection secrets instead of running an interactive OAuth flow.",
...
const RAYCAST_UTILS_IMPORT_SUPPORT = {
  ...
  OAuthService: {
    note: "Jingle supports OAuthService token initialization through withAccessToken; Notion V1 resolves tokens from connection secrets instead of running an interactive OAuth flow.",
    status: "supported-with-migration-note",
    target: "OAuthService"
  },
  showFailureToast: {
...
const JINGLE_DEFAULT_DEPENDENCY_VERSIONS = {
...
function toJingleToolName(raycastName) {
...
function inferLauncherCapabilities(pkg) {
...
function buildDependencyReport(pkg, sourceFiles) {
  ...
  return dependencyNames.map((name) => {
    const decision = DEPENDENCY_DECISIONS[name] ?? {
      ...
      jingleTarget: name.startsWith("@raycast/") ? undefined : name,
      reason: name.startsWith("@raycast/")
        ? "Raycast runtime package needs a Jingle facade or rewrite."
    ...
    return {
      ...
      name,
      jingleTarget: decision.jingleTarget,
...
function extractToolPreview(reader, extensionPath, pkg, target) {
  ...
  return toolFiles.map((file) => {
    ...
    return {
      ...
      jingleName: toJingleToolName(raycastName),
      raycastName,
...
function resolveMemberSupport(importedName, memberName) {
  ...
  if (importedName === "Icon") {
    return SUPPORTED_ICON_MEMBERS.has(memberName)
      ? { status: "supported" }
      : {
          note: `Icon.${memberName} is not in the Jingle icon facade yet.`,
          status: "unsupported"
        }
  }
...
function buildZodSchemaDraft(jingleName, input) {
  if (input.fields.length === 0) {
    return `const ${jingleName}InputSchema = z.object({})`
  }
...
function classifyFeasibility(preview) {
  ...
  return {
    automatic: [
      "package metadata -> Jingle extension manifest draft",
      ...
      "withAccessToken -> Jingle connection secret resolver",
      "src/tools handlers -> generated Jingle tool definitions through the migrated runtime SDK context"
    ],
    manualOrAdapterRequired: [
      "Raycast OAuth interactive authorization -> Jingle interactive connection flow",
      "Raycast UI commands/components -> Jingle runtime SDK components",
      "Raycast toast/navigation/storage hooks -> Jingle host capabilities"
    ],
    score: {
      manifest: "high",
...
export function buildRaycastAiMigrationArtifacts(preview) {
  ...
  return {
    ...
    "jingle-package/main.ts": buildMainPreviewSource(hostPreview, "./main/tools"),
    "jingle-package/identity.ts": buildIdentityPreviewSource(preview),
    "jingle-package/main/tools.ts": buildToolsPreviewSource(hostPreview, "../"),
    "jingle-package/manifest.ts": buildManifestPreviewSource(hostPreview, {
      useCommandMetaImports: true
    }),
    "jingle-package/package.json": `${JSON.stringify(packagePreview, null, 2)}\n`,
    "jingle-package/runtime-metadata.ts": buildRuntimeMetadataPreviewSource(hostPreview),
    "jingle-package/runtime.ts": buildRuntimePreviewSource(hostPreview),
    "jingle-package/tsconfig.check.json": `${JSON.stringify(buildPackageTypecheckConfig(hostPreview), null, 2)}\n`,
    "jingle-package/types.d.ts": buildTypesPreviewSource(hostPreview),
    ...
    "tools.preview.ts": buildToolsPreviewSource(preview, "./jingle-package/"),
    ...
    compilerOptions: {
      ...
      paths: {
        ...
        "@jingle/extension-api": [
          join(JINGLE_REPO_ROOT, "packages/extension-api/src/index.ts")
        ],
        "@jingle/extension-api/host-runtime": [
          join(JINGLE_REPO_ROOT, "packages/extension-api/src/host-runtime.ts")
        ],
        "@jingle/extension-utils": [
          join(JINGLE_REPO_ROOT, "packages/extension-utils/src/index.ts")
        ],
        "@shared/*": [join(JINGLE_REPO_ROOT, "src/shared/*")]
      },
      skipLibCheck: true,
      strict: true,
      target: "ESNext",
      typeRoots: ["./node_modules/@types", join(JINGLE_REPO_ROOT, "node_modules/@types")]
    },
    include
  }
...
function buildDependencyTypecheckPaths(preview) {
  ...
  for (const name of Object.keys(dependencies)) {
    if (name === "react" || name.startsWith("@jingle/")) {
      continue
    }

    const packageRoot = join(JINGLE_REPO_ROOT, "node_modules", name)
...
function buildDependencyReportMarkdown(preview) {
  ...
  const dependencyRows = preview.dependencyReport.map((dependency) =>
    [
      ...
      formatMarkdownCell(dependency.jingleTarget ?? ""),
  ...
  return `${[
    ...
    "## Dependencies",
    "",
    "Name | Category | Decision | Jingle Target | Version | Imported By | Reason",
    "--- | --- | --- | --- | --- | --- | ---",
    ...dependencyRows,
    "",
...
function buildToolsPreviewSource(preview, migratedSourceImportPrefix = "./") {
  ...
  return `${[
    ...
    "",
    hasMigratedToolSources
      ? "// Generated migration preview. Handlers call migrated source tool modules inside a Jingle SDK context."
      : "// Generated migration preview. Fill each handler with migrated Jingle logic before wiring this into an extension package.",
    schemaDefinitions,
    "",
    hasMigratedToolSources
...
function buildManifestPreviewObjectSource(preview) {
  ...
  if (manifest.aiCapability?.id) {
    manifest.aiCapability.id = "__JINGLE_EXTENSION_ID__"
    replacements.set(JSON.stringify("__JINGLE_EXTENSION_ID__"), "EXTENSION_ID")
  }
  if (manifest.aiCapability?.title) {
    manifest.aiCapability.title = "__JINGLE_EXTENSION_TITLE__"
    replacements.set(JSON.stringify("__JINGLE_EXTENSION_TITLE__"), "EXTENSION_TITLE")
  }
  if (manifest.aiCapability?.mention?.value) {
    manifest.aiCapability.mention.value = "__JINGLE_EXTENSION_ID__"
  }
  if (manifest.connection?.provider) {
    manifest.connection.provider = "__JINGLE_EXTENSION_PROVIDER_ID__"
    replacements.set(
      JSON.stringify("__JINGLE_EXTENSION_PROVIDER_ID__"),
      "EXTENSION_PROVIDER_ID"
    )
  }
  if (manifest.connection?.title) {
    manifest.connection.title = "__JINGLE_EXTENSION_TITLE__"
  }
  if (manifest.name) {
    manifest.name = "__JINGLE_EXTENSION_ID__"
  }
  if (manifest.title) {
    manifest.title = "__JINGLE_EXTENSION_TITLE__"
  }
  ...
  for (const command of manifest.commands) {
    ...
    const placeholder = `__JINGLE_COMMAND_VIEWPORT_${command.name}__`
...
function buildRuntimePreviewSource(preview) {
  ...
  const runtimeImport =
    placeholderComponents.length > 0
      ? `import { ${buildRuntimePlaceholderImportNames(commandSourceEntries)}, defineNativeExtensionRuntime } from "@jingle/extension-api"`
  ...
  return [
    ...
    `    markdown: ${JSON.stringify(
      [
        ...
        "",
        "Adapt the migrated Raycast command source to the Jingle extension runtime before promoting this command to a live UI command."
      ].join("\n")
    )},`,
  ...
  if (command.mode === "no-view") {
    ...
    return [
      ...
      `    throw new Error(${JSON.stringify(
        `Migrated no-view command source for "${command.name}" is not wired into the Jingle runtime.`
      )})`,
      "  }",
      "}"
    ].join("\n")
...
function buildTypesPreviewSource(preview) {
  return `${[
    ...
    "",
    "// Generated migration preview types that Jingle injects for migrated extensions.",
    "declare global {",
...
function buildPreferenceTypeAliases(preview) {
  const extensionPreferences = [
    ...
    ...connectionSecretNamesToTypePreferences(getManifestPreviewConnection(preview))
  ]
  const lines = [
...
function getManifestPreviewConnection(preview) {
  const connection = preview.manifestPreview.connection
  if (!connection || typeof connection !== "object") {
    throw new Error("Migration preview manifest is missing connection.")
  }
  return connection
}
...
function buildIdentityPreviewSource(preview) {
  const connection = getManifestPreviewConnection(preview)
  return `${[
    ...
    `  providerId: ${JSON.stringify(connection.provider)},`,
...
function buildCommandContractArtifacts(preview) {
  ...
  for (const command of runtimeCommands) {
    if (command.mode === "view") {
      artifacts[`jingle-package/src/${command.name}.meta.ts`] = buildCommandMetaSource(command)
    }

    const wrapperSource = buildCommandWrapperSource(preview, command)
    if (wrapperSource) {
      artifacts[`jingle-package/src/${command.name}.tsx`] = wrapperSource
    } else if (preview.hostEntryMode === "shell") {
      artifacts[`jingle-package/src/${command.name}.tsx`] = buildCommandShellSource(command)
    }
  }
...
function buildCommandShellSource(command) {
  if (command.mode === "menu-bar") {
    return `${[
      'import { createElement } from "react"',
      'import { MenuBarExtra } from "@jingle/extension-api"',
      "",
  ...
  return `${[
    ...
    `    markdown: ${JSON.stringify(
      [
        ...
        "",
        `Migrated command source for \`${command.name}\` is not wired into the Jingle runtime yet.`,
        "",
...
function buildPackagePreview(preview) {
  if (preview.hostEntryMode === "shell") {
    return {
      name: toJinglePackageName(preview.source.targetExtensionId ?? preview.source.packageName),
      version: "0.0.0",
      private: true,
      type: "module",
      main: "./main.ts",
      ...
      dependencies: sortRecord({
        ...
        react:
          JINGLE_DEFAULT_DEPENDENCY_VERSIONS.react ??
  ...
  const dependencies = Object.fromEntries(
    preview.dependencyReport
      .filter(
        (dependency) => dependency.jingleTarget && dependency.jingleTarget !== "@raycast/api"
      )
      .filter((dependency) => isPackageDependencyName(dependency.jingleTarget))
      .map((dependency) => [
        dependency.jingleTarget,
        dependency.jingleTarget?.startsWith("@jingle/")
          ? "workspace:*"
          : (JINGLE_DEFAULT_DEPENDENCY_VERSIONS[dependency.jingleTarget] ??
            dependency.version ??
            "latest")
      ])
  )

  dependencies["@jingle/extension-api"] = "workspace:*"
  dependencies.zod = JINGLE_DEFAULT_DEPENDENCY_VERSIONS.zod
...
function toJinglePackageName(packageName) {
  const normalized = String(packageName)
    .replace(/^@raycast\//, "")
    .replace(/^@jingle\/extension-/, "")
  return `@jingle/extension-${normalized}`
}
...
function getToolFunctionName(tool) {
  return `${tool.jingleName}Tool`
}

function getToolConfirmationFunctionName(tool) {
  return `${tool.jingleName}Confirmation`
}
...
function buildToolDefinitionSource(tool) {
  const schemaName = `${tool.jingleName}InputSchema`
  ...
  return `${[
    ...
    `  name: ${JSON.stringify(tool.jingleName)},`,
    ...
    .map((tool) => {
      ...
      if (!importPath) {
        return [
          ...
          `  throw new Error(${JSON.stringify(`Migrate ${tool.sourceFile} into a Jingle handler.`)})`,
          "}"
        ].join("\n")
      }
...
function buildSourceMigration(reader, extensionPath, sourceFiles, target) {
  ...
  const rewrittenSourceFiles = sourceFiles.flatMap((file) => {
    ...
    const output = {
      ...
      outputPath: `jingle-package/${file.path}`,
      path: file.path,
    ...
    return [
      output,
      {
        ...
        mainOutputPath,
        notes: file.blockingAdapters,
        outputPath: `jingle-package/${mainOutputPath}`,
        path: file.path,
        sourceText: rewrittenSource.sourceText
      }
    ]
  ...
  const assetFiles = reader.listFiles(posix.join(extensionPath, "assets")).map((path) => ({
    outputPath: `jingle-package/${path}`,
    path,
...
function buildSourceMigrationArtifacts(preview, options = {}) {
  ...
  if (preview.sourceMigration.assetFiles.length === 0) {
    artifacts["jingle-package/assets/.gitkeep"] = ""
  } else {
