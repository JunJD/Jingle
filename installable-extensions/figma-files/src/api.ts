import { getPreferenceValues } from "@jingle/extension-api"
import {
...
function getRetryDelaySeconds(response: Response, attempt: number): number {
...
async function request<T>(
  ...
  async function runRequest(attempt: number): Promise<T> {
    ...
    if (response.status === 429 && attempt < maxRetries) {
      ...
      const retryAfterSeconds = getRetryDelaySeconds(response, nextAttempt)
      await sleep(retryAfterSeconds * 1000)
      return runRequest(nextAttempt)
    }
...
async function fetchProjectFiles(project: FigmaProject): Promise<FigmaProjectFiles> {
  const result = await request<{ files?: FigmaFile[] }>(
    `/projects/${project.id}/files?branch_data=true`,
    {
      method: "GET"
    }
  )
...
export async function resolveAllFiles(): Promise<FigmaTeamFiles[]> {
  ...
  const refreshIds =
    cachedProjects.size === 0 ? allProjectIds : await getProjectsNeedingRefresh(allProjectIds)
