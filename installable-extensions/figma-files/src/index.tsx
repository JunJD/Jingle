...
function OpenSettingsAction(props: { title: string }): React.JSX.Element {
  return (
    <Action
...
export default function FigmaFilesIndex(): React.JSX.Element {
  ...
  const visibleTeams = useMemo(
    () => buildVisibleTeams(allFiles, selectedFilter),
  ...
  const searchBarAccessory = useMemo(
          ...
          <List.Dropdown.Section title="Teams">
            {allFiles.map((team) => (
              <List.Dropdown.Item
                key={team.name}
                title={team.name}
                value={createTeamFilter(team.name)}
              />
            ))}
    ...
    [allFiles, copy.allFiles, selectedFilter]
  )
    ...
    >
      {!hasAccessToken ? (
        <List.EmptyView
          actions={
            <ActionPanel>
              <OpenSettingsAction title={copy.connectFigmaInSettings} />
            </ActionPanel>
          }
        ...
        <List.EmptyView
          actions={
            <ActionPanel>
              <OpenSettingsAction title={copy.addTeamIds} />
            </ActionPanel>
          }
          ...
          actions={
            <ActionPanel>
              <Action
                ...
                title={copy.retry}
              />
              <OpenSettingsAction title={copy.openExtensionSettings} />
            </ActionPanel>
          }
      ...
      {visibleTeams.map((team) =>
        team.files.map((project) => (
          <List.Section
            ...
            subtitle={team.name}
            title={
              project.files.length > 0 ? `${project.name} (${project.files.length})` : project.name
            }
          >
