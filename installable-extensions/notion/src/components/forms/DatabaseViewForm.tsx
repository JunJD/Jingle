...
import { notionColorToTintColor, DatabaseProperty, isType } from "../../../domain"
...
export function DatabaseViewForm(props: {
  ...
  async function handleSubmit(values: Form.Values<any>) {
  ...
  const [name, setName] = useState(databaseView?.name ? databaseView.name : "")
  const { data: databaseProperties, isLoading: isLoadingDatabaseProperties } =
    useDatabaseProperties(databaseId)
  const hasKanbanProperty = databaseProperties.some((dp) => dp.type === "select" || dp.type === "status")
  const [selectedViewType, setSelectedViewType] = useState<"kanban" | "list">(databaseView?.type ?? "list")
  const viewType =
    selectedViewType === "kanban" && !isLoadingDatabaseProperties && !hasKanbanProperty
      ? "list"
      : selectedViewType

  function handleViewTypeChange(value: string) {
    if (value === "kanban" && !hasKanbanProperty) {
      showToast({
    ...
    setSelectedViewType(value as "kanban" | "list")
  }

  return (
    <Form
      isLoading={isLoadingDatabaseProperties || isLoadingDatabases}
      navigationTitle="Set View Type"
      actions={
        <ActionPanel>
          <ActionPanel.Section>
    ...
    >
      ...
      {viewType === "kanban" ? (
        <KanbanViewFormItem
          databaseView={databaseView}
          properties={databaseProperties.filter((dp): dp is StatusDatabaseProperty =>
  ...
  function getStatusState(property: DatabaseProperty | undefined) {
    ...
    const defaultNotStartedOpts = currentConfig
      ...
      : statusOptions[0] && !defaultCompletedOpts.includes(statusOptions[0].id!)
        ? [statusOptions[0].id!]
        : []
    let defaultStartedOpts = currentConfig ? currentConfig.started_ids : []
    if (!currentConfig) {
      const defaultNotStartedOptSet = new Set(defaultNotStartedOpts)
      const defaultCompletedOptSet = new Set(defaultCompletedOpts)
      defaultStartedOpts = []
      for (const option of statusOptions) {
        if (!option.id || defaultNotStartedOptSet.has(option.id) || defaultCompletedOptSet.has(option.id)) {
          continue
        }
        defaultStartedOpts.push(option.id)
      }
    }
