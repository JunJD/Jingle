...
void React
import { Form, Icon, Image } from "@jingle/extension-api";
import type { useForm } from "@jingle/extension-utils";
...
export function PagePropertyField({
  ...
  switch (type) {
    case "date":
      return <Form.DatePicker key={itemProps.id} {...itemProps} {...sharedProps} />;
    case "checkbox":
      return <Form.Checkbox key={itemProps.id} {...itemProps} {...sharedProps} label={sharedProps.placeholder} />;
    case "select":
    case "status":
      return (
        <Form.Dropdown key={itemProps.id} {...itemProps} {...sharedProps}>
    ...
    case "people": {
      ...
      return (
        <Form.TagPicker key={itemProps.id} {...itemProps} {...sharedProps}>
...
type ItemOption = DatabasePropertyConfig<"select">["options"][number];
function createMapOptionsFunc(Tag: typeof Form.Dropdown.Item | typeof Form.TagPicker.Item) {
  return function MapOption(option: ItemOption | Page | User) {
