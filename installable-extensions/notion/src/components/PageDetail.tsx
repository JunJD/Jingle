...
void React
import { ActionPanel, Detail, Action, Icon, Image, Color, getPreferenceValues } from "@jingle/extension-api";
import { useCachedPromise } from "@jingle/extension-utils";
import { format, formatDistanceToNow } from "date-fns";
import type { ReactNode } from "react";
import { useEffect, useMemo, useState } from "react";

import { fetchPageContent, getPageName, notionColorToTintColor, PageProperty, Page, User } from "../../domain";
...
function pagePropertyToText(
  ...
  switch (property.type) {
    ...
    case "formula":
      return pagePropertyToText({ ...property.value, id: property.id }, users);
    case "multi_select":
    ...
    case "people":
      return property.value
        .map((person) => users?.find((user) => user.id === person.id)?.name ?? person.id)
        .join(", ");
    case "rich_text":
    ...
    case "title":
      ...
      return;
    case "relation":
      return property.value.map((relation) => relation.id).join(", ");
    default:
...
export function PageDetail({ page, setRecentPage, users }: PageDetailProps) {
  ...
  const detailMetadata = useMemo(
    ...
    [createdByUser, lastEditedUser, page.last_edited_time, page.properties, users]
  )
  const appendContentTarget = useMemo(
