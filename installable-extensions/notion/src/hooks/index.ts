import { LocalStorage, showToast } from "@jingle/extension-api"
...
export function useVisibleDatabasePropIds(
  ...
  const [quicklinkVisiblePropIds, setQuicklinkVisiblePropIds] = useState(quicklinkProps)
  const { data, isLoading, setDatabaseView } = useDatabasesView(databaseId)

  if (quicklinkProps) {
    return {
      visiblePropIds: quicklinkVisiblePropIds,
      isLoading: false,
      setVisiblePropIds: setQuicklinkVisiblePropIds
    }
  }
...
export function useDatabasesView(databaseId: string | null) {
...
export function usePinnedPages() {
  ...
  async function setPinnedPage(page: Page) {
    const storageData = await LocalStorage.getItem("PINNED_PAGES")
    const pinnedPages: PinnedPage[] =
      storageData && typeof storageData === "string" ? JSON.parse(storageData) : []
