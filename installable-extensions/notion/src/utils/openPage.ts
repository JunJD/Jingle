...
import { closeMainWindow, getPreferenceValues, open } from "@openwork/extension-api"

import { Page } from "../../domain/page"

type NotionExtensionPreferences = {
...
export async function handleOnOpenPage(page: Page, setRecentPage: (page: Page) => Promise<void>): Promise<void> {
  if (!page.url) return
  const open_in = getPreferenceValues<NotionExtensionPreferences>().open_in
 ...
 */
export function isNotionApp(app?: RuntimeOpenApplication): boolean {
  if (!app) return false
...
export function urlForPreferredMethod(url: string, open_in?: RuntimeOpenApplication) {
