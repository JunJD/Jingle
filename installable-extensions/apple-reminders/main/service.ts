...
import { defineNativeExtensionService, type IpcErrorCode } from "@jingle/extension-api"
import {
...
const APPLE_REMINDERS_HELPER_NAME = "jingle-apple-reminders"
const APPLE_REMINDERS_HELPER_INFO_PLIST_NAME = "jingle-apple-reminders-info.plist"
...
function resolveAppleRemindersSwiftSourcePath(): string | null {
  const candidates = [
    join(app.getAppPath(), "src/native/jingle-apple-reminders.swift"),
    join(process.cwd(), "src/native/jingle-apple-reminders.swift"),
    join(
    ...
    join(app.getAppPath(), "out/native/jingle-apple-reminders.swift"),
    join(process.cwd(), "out/native/jingle-apple-reminders.swift"),
    join(appleRemindersMainModuleDir, "..", "native", "jingle-apple-reminders.swift")
  ]
...
export function normalizeAppleRemindersError(error: unknown): AppleRemindersRequestError {
  ...
  if (
    message.includes("JingleRemindersAccessDenied") ||
    message.includes("not authorized") ||
    message.includes("not authorised") ||
    message.includes("authorization denied")
  ) {
    return new AppleRemindersRequestError(
      "Jingle needs permission to access Reminders. Grant Reminders access in System Settings and try again.",
      "PERMISSION_DENIED"
    )
  }

  if (message.includes("JingleReminderNotFound")) {
    return new AppleRemindersRequestError(
  ...
  if (message.includes("JingleReminderListNotFound")) {
    return new AppleRemindersRequestError(
  ...
  if (message.includes("JingleUnsupportedMethod")) {
    return new AppleRemindersRequestError(
      "Jingle could not complete the Reminders request. Restart Jingle and try again."
    )
  }
