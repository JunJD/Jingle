...
Openwork 的 bundled extension 正在迁到 monorepo 根目录的 `installable-extensions/<extension>`。这样做的目标不是提前做 marketplace，而是让 Apple Reminders、Figma Files、GitHub、Notion 这类 extension 先按未来外部安装包的约束运行，并具备同一套边界：
...
`src/extensions` 是宿主侧 registry/loader 层，不再作为新 extension package 的默认源码根。还没有迁出的旧内置 extension 可以短期留在 `src/extensions/<extension>`，但新增或迁移的 extension 应落在 `installable-extensions/<extension>`。
...
`npm run check:extensions` 会检查 bundled `installable-extensions/<extension>` package 的最小包形态和 import 边界：

- package 目录名必须和 `package.json` 的 Openwork package name 对齐：`installable-extensions/<extension>` 必须声明 `"name": "@openwork/extension-<extension>"`。
...
这些 registry 是 built-in 装配细节，不是 extension package 应该依赖的 API。Installable extension 由 `openwork.extension.json` 进入 installed provider，再经 registry service 投影给 main/preload/renderer；每个 package 暴露的 manifest、runtime entry、main entry 和 JSON-safe runtime metadata contract 应保持一致。
