...
CLI 发布的 runtime bundle 和 privileged main bundle 分别使用内容寻址文件名 `dist/runtime-<sha256>.mjs` 与 `dist/main-<sha256>.mjs`。`jingle.extension.json.runtimeArtifactRevision` / `mainArtifactRevision` 必须是对应文件精确字节的 `sha256:<64 lowercase hex>`；installed provider 会同时校验 descriptor 格式、文件名和实际内容。缺少 runtime revision 的旧 descriptor 只能投影为显式 unavailable；声明 main entry 却缺少 main revision 的 package 必须 fail closed。宿主不能用 package version、路径或 mtime 猜测任何 artifact identity。

Privileged main 的 verified revision 必须绑定实际被求值的 bytes：registry 在一次读取中计算 digest 并携带该 bytes，main loader 从这份 immutable input 执行，不在验证后重新按文件路径 import。main loader 的 module cache identity 使用 verified revision。该 revision 证明内容 identity 与完整性，不证明发布者身份，也不替代包签名或 trust policy。
