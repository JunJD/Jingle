...
第二，Openwork 应该认真学习 Craft 的 Source 和 Permission Mode。  
Source 让 Agent 的上下文有边界，Permission Mode 让风险变成人能理解的选择。Explore、Ask to Edit、Auto 这类语言，比“工具 X 是否 allow”更像产品。

第三，Openwork 要把 Craft 的 Source 放进自己的 harness。  
这一步很关键。Source 如果只存在于设置页，它只是配置。Source 如果进入 run metadata，进入审批记录，进入 artifact 和 replay，它才变成信任的一部分。

第四，Openwork 的 Source UI 要尽快变成用户能感知的东西。  
如果只有内部架构，没有 source profile list、source picker、auth 状态、per-run source binding，用户不会知道系统边界在哪里。没有边界感，Agent 越强，用户越紧张。
