...
CopilotKit 的 agent 设计更像“Web runtime + agent protocol + React SDK”。Openwork 的 agent 设计应该是“本地主权 runtime + IPC event API + desktop capability kernel + renderer headless core”。
...
这层可以继续保留 DeepAgents/LangGraph，只是把它变成 `adapters/deepagents.ts`。真正要先定义清楚的是 Openwork-owned kernel API，而不是马上换掉 kernel 实现。
...
这层可以借鉴 CopilotKit `connect/run/stop/threads` 的入口拆法，但不应该变成 HTTP runtime。HTTP/SSE 最多作为未来 remote adapter，不能倒过来决定 Openwork runtime API。
...
| 当前文件 | 目标归属 | 迁移动作 |
| --- | --- | --- |
| `src/shared/agent-thread-runtime.ts` | 暂留 shared，未来 protocol | Phase 2 先保持事实源，runtime/core 同时依赖；稳定后再迁包 |
...
这里故意使用少量 `unknown` 和 string 类型，是为了 PR1 不依赖 `src/shared`、`src/main`、`src/renderer`。到 Phase 2 再把 `AgentThreadEventBatch`、`HITLDecision`、`ComposerMessageInput` 等真实 shared 类型接进来。
...
- 当前 Runtime v2 event 语义，至少 Phase 2 不重命名核心 event。
...
- 保持现有 IPC channel 不变。
- 给 `RuntimeEventProjector` 增加小型边界测试，覆盖 invoke、resume、approval、cancel、error。
...
| 过早拆第三个 protocol 包 | 迁移面变大，类型引用更乱 | Phase 2 先稳定 shared event API，再决定迁出 |
...
| 原目标要求 | 当前报告覆盖 | 仍缺什么 |
| --- | --- | --- |
| 增加一个后端 `agent-runtime` package | `packages/agent-runtime` 已创建，包含 `package.json`、`README.md`、`src/index.ts`，并在 `tsconfig.node.json` 增加 `@openwork/agent-runtime` path | 后续 Phase 1/3 再接 trace 和真实 runtime 行为 |
| 增加一个前端 `agent-core` package | `packages/agent-core` 已创建，包含 `package.json`、`README.md`、`src/index.ts`，并在 `tsconfig.web.json` 增加 `@openwork/agent-core` path | 后续 Phase 4 再接真实 renderer core 行为 |
| message 和输入框基础样式保留 | `重构范围` 明确 Chat message、输入框基础样式、键盘体验、附件/refs 入口保留 | 后续 Phase 4/5 接入 core 时需要代码验证 UI 未重做 |
...
- 暂不单独建 `agent-tracing` 包；run trace 先作为 `agent-runtime` 的 tracing 能力接入现有 runtime。等它被 runtime、CLI/diagnostic、前端 debugger 同时使用后，再判断是否独立成包。
...
- `ThreadRuntimeProjector` 最终归 `agent-runtime` 还是独立 protocol-adjacent 模块，需要在 Phase 2 根据测试边界决定。
...
- `ThreadContext` 最终是完全消失，还是作为 `AgentCoreProvider` 的 app bridge 保留，要等 Phase 4 拆 `useAgent` 后判断。
...
2. 先给现有 runtime 铺 run trace，让每次 invoke/resume/HITL/cancel 都能被 inspect。
3. 后端先保留 DeepAgents/LangGraph adapter，不急着换 kernel。
4. 前端再拆 headless core，保留 message/input 基础样式。
5. tool/HITL renderer registry 作为第一批明显用户可见收益。
6. 等包边界稳定后，再评估 Flue/PI adapter、AG-UI/CopilotKit event 形态是否适合进一步吸收。
