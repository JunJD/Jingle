...
Jingle uses one SQLite database accessed through Prisma.
...
- [human-approval-middleware.ts](../packages/langchain-agent-harness/src/human-approval-middleware.ts)
- [runtime-assembly.ts](../src/main/agent/runtime-assembly.ts)
- [tool-permission-runtime.ts](../src/main/agent/tool-permission-runtime.ts)

Important invariant:

- approval interruption happens inside the harness `wrapToolCall` path
- `execute` policy/review ownership stays in main
...
If approvals stop being durable, Jingle stops being a trustworthy harness.
...
- [persistence.ts](../src/main/agent/persistence.ts)
- [runtime-state.ts](../src/main/agent/runtime-state.ts)
...
- [controller.ts](../src/main/agent/controller.ts)
