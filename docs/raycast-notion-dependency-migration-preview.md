...
4. 历史 `notion-generated` preview 验证了哪些语义，以及这些语义如何被吸收到正式 `installable-extensions/notion`。

当前生产入口只有正式 `installable-extensions/notion`。`notion-generated` 只作为历史 preview 证据出现，不是新的开发入口，也不应重新接入 registry。
...
生产入口只保留正式 `installable-extensions/notion`。`notion-generated` 不再是 bundled extension，不进入 manifest/main/runtime/runtime-metadata registry，也不会出现在 launcher、settings、menu-bar 或 AI capability 列表里。

正式入口：

- `installable-extensions/notion` 是迁移后的正式 Notion package，extension id、title、runtime metadata、AI capability、settings schema 都固定为 `notion`。
...
- 旧 `openwork://extensions/notion-generated/<command>` quicklink 不再归一化为正式 `notion` 链接；quicklink repository 会清理未知 extension 链接，直接注册未知 extension 会失败。
...
- 真实 Electron/CDP 已验证：Settings schema 只包含 `notion`；launcher 搜索 `notion` 显示正式 Notion 四个 command；旧 `notion-generated` quicklink 不再迁移到正式 Notion。
...
- 正式 `notion` AI tools 覆盖 search/read/write、page markdown、分页 block 读取和 Notion property wrappers；旧 RPC/service 测试已移除，AI 工具直接走 `main/tools.ts` 的 Notion client/domain helper。
- 正式 `installable-extensions/notion` 已移除旧 RPC surface：manifest 不再声明 `rpc` capability/runtimeCapability/rpcMethods，`main.ts` 不再导出 service，旧 `main/service.ts`、`main/client.ts`、`src/runtime-client.ts` 和旧 RPC contracts 已删除。
- 正式 `installable-extensions/notion` 已把身份集中到 package 级 `identity.ts`，并固定为正式 `notion` identity：manifest/runtime metadata、AI tool host request id、runtime quicklink URL 都指向正式 `notion`。
...
./node_modules/.bin/tsc --noEmit -p installable-extensions/notion/tsconfig.check.json --pretty false
...
- `installable-extensions/<name>` 是 extension package 本身，承载 manifest、commands、AI tools、business helpers、assets。
...
第一波迁移后，Apple Reminders、Figma Files、GitHub、Notion 这类具体 extension package 应落在 `installable-extensions/<name>`。`src/extensions` 保留为显式 bundled registry/loader 层，并临时承载尚未迁出的旧内置 extension；不引入自动扫描和 install 系统。
...
对 `installable-extensions/notion/src` 做 `git grep` 后可以确认：
...
node scripts/preview-raycast-ai-migration.mjs \
  ...
  --extension-path installable-extensions/notion \
...
这个命令形态曾用于把官方 Raycast Notion 迁移产物作为 `notion-generated` 并行接入，避免预览包和正式 `notion` extension 抢同一个 manifest id。当前生产状态只保留正式 `installable-extensions/notion`；旧 preview 产物只保留在 `.ignored-extensions/notion-generated-preview/` 作为本机历史对照，不再进入 registry。
...
- 旧 `notion-generated` 预览包已经退出生产 registry；遗留 generated quicklink URL 会被清理或显示为未知 extension，不会再产生新的 generated launcher/settings 入口。
...
说明：下面的 `notion-generated` 条目是历史 preview package 的迁移证据，只用于说明哪些 Raycast Notion 语义已经被验证并吸收到正式 `installable-extensions/notion`；当前生产入口仍只有正式 `notion`。
...
- Raycast 的 `openExtensionPreferences` / `openCommandPreferences` 会在迁移产物里重写为 `openNativeExtensionSettings` / `openNativeCommandSettings`，并归到 `settings` runtime capability。
...
- 官方 Raycast Notion 迁移产物已经晋升为正式 `installable-extensions/notion`，manifest/main/runtime/runtime-metadata 四件套由正式包被宿主消费。
- 旧 `notion-generated` 已退出生产 registry，不再复用连接、不再暴露 AI capability；历史 quicklink 不再通过兼容层迁到正式 `notion`。
...
- `notion-generated` 曾经验证过迁移期 AI tool 面；正式 `notion` 当前只保留 `searchPages/getPage/getPageMarkdown/addToPage/retrievePage/listBlockChildren/retrieveDataSource/queryDataSource/createDatabasePage` 等当前 tool contract，不再保留 `createPage/searchDatabase` 旧 tool 名。
...
- 官方 Raycast Notion 迁移产物可以通过生成包自己的 `tsconfig.check.json`，并在 symlink 到 `installable-extensions/<id>` 后通过 `validateNativeExtensionPackageBoundaries`。
...
1. 继续按正式 `installable-extensions/notion` 往下跑未覆盖 command/action，优先补真实 smoke 暴露的 facade 缺口，而不是提前追完整 Raycast runtime。
