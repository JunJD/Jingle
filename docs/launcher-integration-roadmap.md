...
- `files` source 已开始接入：当前先支持 macOS 下基于 Spotlight 的文件 / 目录搜索，结果进入现有 `search-results` section，执行先复用 `open-path`
- 后续 `Source` backlog：
  - `BrowserBookmarksLauncherSearchProvider`
  - `ClipboardIntentSource`
  - 这几类统一按 `class X implements LauncherSearchProvider` 的风格落地，并 export 稳定 provider 实例，只在 source 层类化，不向 `Candidate / SurfaceModel / Action` 扩散
