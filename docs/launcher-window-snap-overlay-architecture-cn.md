...
自管拖拽不会在 mouseDown 后立刻开始。用户需要在可拖区域按住 `500ms`，随后窗口才进入拖动状态，参考线和吸附规则也在同一刻启用。
...
所以这里不做几秒级长按，也不让参考线 mouseDown 秒出。当前选择是：按住 `500ms` 后进入 launcher 自管拖拽，参考线和吸附规则同步启用。
...
sequenceDiagram
  ...
  Main->>Main: wait 500ms before activating drag
...
- 吸附阈值：`8px`。
- 拖拽激活延迟：`500ms`，延迟结束前不移动窗口、不显示参考线，也不应用吸附。
