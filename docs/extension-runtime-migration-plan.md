...
Openwork 当前正在把内置在 renderer 里的 native extension command，逐步迁移到独立的 extension runtime 模型。这个工作现在的目标已经从“内置 command 迁移”升级为 “External Extension Contract v1”：内置 extension 要先按未来外部 extension 的规则运行，即使物理上仍保留在 monorepo 里。
...
3. manifest 是 command、mode、preferences、runtime capabilities、source/tool 声明的事实来源
4. 使用 `ai`、`clipboard`、`rpc`、`storage`、`shell`、`navigation` 等 host capability 必须在 manifest 声明，并由 runtime manager 硬校验
5. human launcher command 和 assistant source/tool 分层：前者服务 `view` / `no-view` / `menu-bar`，后者服务 agent-visible guide、tool schema、permission、profile

Raycast 给 Openwork 的参考是 command runtime 和 extension author API：`view` / `no-view` / `menu-bar` 生命周期、manifest command source of truth、`AI.ask` 这类 host API。Craft Source 给 Openwork 的参考是 assistant-facing source/tool/profile/credential 分层。Openwork extension 可以同时承载这些声明，但不能把它们合成一个大对象。
...
1. `src/extensions/<extension>/manifest.ts` 是事实来源，声明 command 是否 runtime、command name、mode、viewport、preferences、capability。
2. `src/extensions/<extension>/runtime.ts` 是 package-level runtime entry，只 import 本 extension 内部的 command component 或 no-view runner，并通过 `defineNativeExtensionRuntime(...)` 暴露：
...
3. `src/extensions/runtime-packages.ts` 是当前 build-time 聚合点。它只 import `./<extension>/runtime` package entry，不 import `./<extension>/src/<command>`。
4. `src/extensions/runtime.ts` 只把 package runtime entries 展开成 `${extensionName}:${commandName}` lookup map，供 `src/extension-runtime/entry.ts` 启动 runtime command。
5. renderer 只能 import manifest、runtime metadata 和 runtime snapshot/surface，不 import runtime command module。搜索增强这类 renderer 可读 metadata 放在 `src/extensions/<extension>/runtime-metadata.ts`，再由 `src/extensions/runtime-metadata-packages.ts` 聚合。
