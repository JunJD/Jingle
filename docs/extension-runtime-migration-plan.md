...
Apple Reminders 的 `menu-bar-reminders` 和 GitHub 的 `unread-notifications` 仍保留在 legacy renderer path，等待 menu bar runtime 调度链路接入后再迁。Translate 的主视图仍需要按下面计划处理，`translate-quick-copy` 已经通过 runtime no-view command 执行。
...
### Step 4: Translate 主视图暂留 legacy renderer path

Translate 主视图当前依赖 `useAI` / `useI18n` 等 renderer/AI 边界。在 AI bridge 定义清楚前，不强迁。`translate-quick-copy` 不渲染 UI，已经迁入 runtime no-view path，用来验证 RPC、navigation 和 clipboard capability 的最小链路。

这样做的影响：

- Translate 主视图继续可用，不阻断当前产品功能
- runtime 迁移不会被 AI/i18n 视图边界拖慢
- quick-copy 的翻译、复制和返回 launcher 由 runtime / main host capability 执行
- 但 runtime 还不能宣称覆盖所有 extension UI 类型

Translate 主视图迁移前必须先定义：
...
- `useAI` / `useI18n` 还没有 runtime 版本，Translate 主视图仍不能迁入 runtime
