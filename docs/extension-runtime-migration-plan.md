...
Apple Reminders 的 `menu-bar-reminders` 和 GitHub 的 `unread-notifications` 仍保留在 legacy renderer path，等待 menu bar runtime 调度链路接入后再迁。Translate 的主视图已经迁入 runtime；曾用于验证 no-view 链路的 quick-copy command 已移除，避免在 launcher 里出现重复且不清晰的翻译入口。
...
Translate 主视图已经通过 runtime command 运行，翻译调用走 AI host capability，复制走 clipboard host capability。旧的 quick-copy no-view command 已移除，launcher 只保留一个清晰的 Translate 入口。
