...
当前 runtime-backed 注册表是：

- `src/extensions/runtime-backed.ts`
...
- renderer 不 import runtime-backed command module
...
### Step 1: Todo 保持 runtime-backed 可用

Todo 是第一条真实链路，用来验证 runtime 基础层。

验收条件：

- `todo-list:index` 在 `src/extensions/runtime-backed.ts` 注册
...
- command 登记到 runtime-backed 注册表
...
- runtime-backed command 的错误 UI 需要统一
- 大列表性能还没有真实压力测试
...
后续判断一个 command 是否已经迁入 runtime，以 `src/extensions/runtime-backed.ts` 为准。
