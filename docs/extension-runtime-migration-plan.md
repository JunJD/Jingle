...
- 正式 package home：`extensions/*/manifest.ts`
- 仍留在 app 内部的 bundled package：`src/extensions/*/manifest.ts`
...
1. `extensions/<extension>/manifest.ts` 或 `src/extensions/<extension>/manifest.ts` 是事实来源，声明 command 是否 runtime、command name、mode、viewport、preferences、capability。Apple Reminders、GitHub、Notion 已在根 `extensions/` 下，todo-list、translate 仍在 `src/extensions/` 下。
...
3. `src/extensions/runtime-packages.ts` 是当前 build-time 聚合点。它只 import package-level `runtime.ts` entry，不 import package 内 `src/<command>`。
...
5. renderer 只能 import manifest、runtime metadata 和 runtime snapshot/surface，不 import runtime command module。搜索增强这类 renderer 可读 metadata 放在 package-level `runtime-metadata.ts`，再由 `src/extensions/runtime-metadata-packages.ts` 聚合。
...
后续判断一个 command 是否已经迁入 runtime，以对应 `extensions/*/manifest.ts` 或 `src/extensions/*/manifest.ts` command 上的 `runtime` 字段为准。
