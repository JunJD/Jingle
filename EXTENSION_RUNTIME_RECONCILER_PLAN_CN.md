...
- Todo List command 能进入 runtime-backed surface。
...
4. 新增 architecture check：renderer 不得 import runtime-backed extension command modules。
...
1. Todo command 改为 runtime-backed route。
...
1. `my-reminders`、`create-reminder` 切 runtime-backed route。
...
1. GitHub view commands 切 runtime-backed route。
...
1. 删除 runtime-backed extension 对 `NativeExtensionPassiveCommandHosts` 的依赖。
...
3. 加强 guardrail：runtime-backed extension 禁止 `window.api`、`window.localStorage`、`@renderer`、`@extension-host`。
...
- GitHub / Apple Reminders / Todo / Translate 都 runtime-backed。
