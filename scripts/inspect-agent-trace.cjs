...
function getJingleHomeResolution() {
  const jingleHome = process.env.JINGLE_HOME?.trim()
  if (jingleHome) {
    return {
      databaseFileName: "jingle.sqlite",
      dir: jingleHome
    }
  }
  ...
  return {
    databaseFileName: "jingle.sqlite",
    dir: path.join(homedir(), ".jingle")
  }
}

function getDbPath() {
  const resolution = getJingleHomeResolution()
  return path.join(resolution.dir, resolution.databaseFileName)
}

function resolveTraceIdArgument(rawTraceId) {
  if (rawTraceId === undefined || rawTraceId.length === 0) {
    return "latest"
  }
  return rawTraceId
}

function readStepArgument(rest) {
  const stepFlagIndex = rest.indexOf("--step")
  if (stepFlagIndex < 0) {
    return null
  }
  return rest[stepFlagIndex + 1]
}

function usage() {
  console.log(`
inspect-agent-trace - Inspect local Jingle agent traces
...
function readJsonQuery(sql, params = []) {
  ...
  if (!existsSync(dbPath)) {
    throw new Error(`Jingle database not found at ${dbPath}`)
  }
...
function handleTraceCommand(traceArgs) {
  ...
  const traceId = resolveTraceId(resolveTraceIdArgument(rawTraceId))
  const trace = getTrace(traceId)
  const stepArg = readStepArgument(rest)
