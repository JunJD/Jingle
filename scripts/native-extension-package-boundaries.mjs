...
const allowedExtensionPackages = new Set(["@jingle/extension-api", "@jingle/extension-utils"])
const forbiddenSourceRuntimePackages = new Map([
  ["@raycast/api", "@jingle/extension-api"],
  ["@raycast/utils", "@jingle/extension-utils"]
])
...
function validatePackageShape({ extensionDirectory, packageJson, packageJsonPath, repoRoot }) {
  ...
  const expectedPackageName = `@jingle/extension-${extensionId}`
...
function readMainEntryDefinitionShape({ extensionDirectory }) {
...
function validateImport({ declaredPackages, extensionDirectory, file, importEntry, repoRoot }) {
  ...
  const extensionTarget = forbiddenSourceRuntimePackages.get(packageName)
  if (extensionTarget) {
    return {
      file: repoFile,
      import: specifier,
      line: importEntry.line,
      reason: `extension package cannot import source runtime package ${packageName}; use ${extensionTarget}`
    }
  }
  ...
  if (allowedExtensionPackages.has(packageName) || mainOnlyPackages.has(packageName)) {
