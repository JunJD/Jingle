...
export interface NativeExtensionSourceMentionProjection {
  ...
  sourceId: string
  supportedPlatforms?: NativeExtensionSupportedPlatform[]
  tools: NativeExtensionSourceMentionToolProjection[]
  value: string
}

export interface NativeExtensionSourceMentionToolProjection {
...
export function listMissingRequiredNativeExtensionPreferences(
  ...
  for (const preference of schema) {
    ...
    if (!isMissingRequiredNativeExtensionPreferenceValue(values[preference.name])) {
      continue
    }

    missingPreferences.push(getNativeExtensionPreferenceDisplayName(preference, locale))
  }
...
export function toLauncherCommandOwnerManifest(
  ...
  return {
    ...
    commands: launcherCommands.map((command) => ({
      ...
      mode: command.mode,
      name: command.name,
      requiresLauncherArguments: command.requiresLauncherArguments,
      title: command.title
    })),
