...
export const FILE_MUTATION_RESULT_METADATA_KEY = "jingleFileMutation"
...
function readFileMutationResultPayload(
  ...
  return metadata[FILE_MUTATION_RESULT_METADATA_KEY]
}
...
export function readFileMutationResultMetadata(
  ...
  const value = readFileMutationResultPayload(message.metadata)
  if (!isRecord(value)) {
    return null
  }
