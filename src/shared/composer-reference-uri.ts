export const EXTENSION_SOURCE_REFERENCE_SCHEME = "jingle-extension-source"
export const WORKSPACE_FILE_REFERENCE_SCHEME = "jingle-workspace-file"
...
const COMPOSER_REFERENCE_MARKDOWN_PATTERN =
  /\[(@[^\]\n]+)\]\((jingle-extension-source|jingle-workspace-file):\/\/([^)\s]+)\)/g
...
function parseComposerReference(
  label: string,
  scheme: string,
  payload: string
): ParsedExtensionSourceReference | ParsedWorkspaceFileReference | null {
  if (isExtensionSourceReferenceScheme(scheme)) {
    return parseExtensionSourceReference(label, payload)
  }

  if (isWorkspaceFileReferenceScheme(scheme)) {
    return parseWorkspaceFileReference(label, payload)
  }
...
export function parseComposerReferenceText(text: string): ParsedComposerReferenceText | null {
  ...
  for (const match of text.matchAll(COMPOSER_REFERENCE_MARKDOWN_PATTERN)) {
    ...
    const reference = parseComposerReference(label, scheme, payload)
...
function isExtensionSourceReferenceScheme(scheme: string | undefined): boolean {
  return scheme === EXTENSION_SOURCE_REFERENCE_SCHEME
}

function isWorkspaceFileReferenceScheme(scheme: string | undefined): boolean {
  return scheme === WORKSPACE_FILE_REFERENCE_SCHEME
}
