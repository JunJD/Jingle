...
export function resolveShortcutBindings(
  ...
  return listDefaultShortcutBindings()
    .filter((binding) => matchesShortcutPlatform(binding.platform, platform))
    .flatMap((binding) => {
      const override = resolveShortcutOverride(
      ...
      return [
        {
          ...binding,
          ...(override?.chord ? { chord: override.chord } : {}),
          source: override?.chord ? "override" : "default"
        }
      ]
    })
}
