...
extension NSScreen {
    ...
    static var jinglePreferredIslandScreen: NSScreen? {
        if let builtinScreen = screens.first(where: { $0.jingleBuiltinDisplay }) {
...
final class IslandController: NSObject {
    ...
    private func panelOrigin(for size: NSSize) -> NSPoint {
        guard let anchorRect = statusItemAnchorRect() else {
            let screen = NSScreen.jinglePreferredIslandScreen
    ...
    private func screenFrame(containing rect: NSRect) -> NSScreen {
        NSScreen.screens.first(where: { $0.frame.intersects(rect) })
            ?? NSScreen.jinglePreferredIslandScreen
            ?? NSScreen.main
            ?? NSScreen.screens[0]
    }
