...
export class FileReferenceNode extends DecoratorNode<null> {
  __label: string
  __name: string
  __path: string
  ...
  updateDOM(prevNode: FileReferenceNode, dom: HTMLElement): false {
...
export function $createFileReferenceNode(payload: FileReferencePayload): FileReferenceNode {
  return $applyNodeReplacement(new FileReferenceNode(payload))
}
