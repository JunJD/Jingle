...
const OAUTH_TOKEN_KEY_PREFIX = "openwork-oauth-token:"
const OAUTH_CLIENT_ID_OVERRIDE_PREFIX = "openwork-oauth-client-id:"
...
export function buildOAuthRedirectUri(redirectMethod: string, extensionName?: string): string {
  const pkg =
    extensionName ||
    getOAuthRuntimeDeps().getExtensionContext().extensionName ||
    "openwork-extension"
  switch (redirectMethod) {
    ...
    case "appURI":
    default:
      return `openwork://oauth/callback?packageName=${encodeURIComponent(pkg)}`
  }
}
...
export async function buildAuthorizationRequest(params: {
  ...
  url.searchParams.set(
    "redirect_uri",
    buildOAuthRedirectUri(params.redirectMethod, params.extensionName)
  )
...
export function parseOAuthCallbackUrl(rawUrl: string): {
  ...
  try {
    ...
    if (parsed.protocol !== "openwork:") return null
