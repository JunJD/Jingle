  ...
  return (Component: any) => {
    const WrappedComponent: React.FC<any> = (props) => {
      ...
      const handleOAuthSignIn = useCallback(async () => {
        ...
        if (!clientId) {
          setOauthHint('Enter an OAuth Client ID to continue.');
          return;
        }
        ...
        setOauthHint('');
        try {
          options.setClientIdOverride(clientId);
      ...
      if (oauthNeedsAuth) {
        ...
        const requiresClientId = !oauthClientIdInput.trim();
          ...
          <div className="h-full flex flex-col">
            <div className="flex-1 flex items-center justify-center px-6">
              <div className="w-full max-w-[520px] text-center">
                ...
                {requiresClientId ? (
                  <div className="mx-auto mb-4 max-w-[360px] text-left">
                    ...
                    <input
                      ...
                      placeholder={`Enter your ${oauthInfo?.name || 'provider'} OAuth Client ID`}
                      className="w-full rounded-lg border border-white/[0.08] bg-white/[0.04] px-3 py-2.5 text-sm text-white outline-none transition-colors placeholder:text-white/25 focus:border-cyan-300/60"
                      autoCapitalize="off"
                      autoCorrect="off"
                      spellCheck={false}
                    />
                    <div className="mt-2 text-xs text-white/40">
                      Openwork does not ship a shared {oauthInfo?.name || 'provider'} OAuth app. Paste your own client ID to continue.
                    </div>
                  </div>
                ) : null}
                <button
