...
import {
  ...
  resolveTintColor,
  toScAssetUrl
} from "./icon-runtime-assets"
import {
  configureOAuthRuntime,
  OAuth,
  OAuthService,
  withAccessToken,
  getAccessToken,
...
import { preferences, updateCommandMetadata, DeeplinkType, createDeeplink } from "./misc-runtime"
import {
  getFavicon,
  getAvatarIcon,
  getProgressIcon,
...
export { WindowManagement, WindowManagementDesktopType, BrowserExtension, executeSQL, withCache }
export type {
...
type RuntimePreferenceDefinition = NonNullable<
  ExtensionContextType["preferenceDefinitions"]
>[number]
...
export class Toast {
  ...
  private updateActions() {
    this._actions = [this._primaryAction, this._secondaryAction].filter(
      (action): action is Alert.ActionOptions => Boolean(action?.title)
    )
  ...
  private matchesActionShortcut(event: KeyboardEvent, action?: Alert.ActionOptions): boolean {
    ...
    const expectedKeyRaw = String(shortcut.key || "")
      .trim()
      .toLowerCase()
    const pressedKeyRaw = String(event.key || "")
      .trim()
      .toLowerCase()
  ...
  private renderActionMenu() {
    ...
    this._selectedActionIndex = Math.max(
      0,
      Math.min(this._selectedActionIndex, this._actions.length - 1)
    )
...
function shouldSuppressBenignGitMissingPathToast(options: Toast.Options): boolean {
  ...
  const isFailure =
    style === ToastStyle.Failure || style === Toast.Style.Failure || style === "failure"
...
function parseClipboardPayload(
  ...
  if (content && typeof content === "object") {
    ...
    const hasStructuredClipboardFields =
      maybeContent.text !== undefined ||
      maybeContent.file !== undefined ||
      maybeContent.html !== undefined
...
export function getPreferenceValues<Values extends PreferenceValues = PreferenceValues>(): Values {
  ...
  const preferenceDefinitions = Array.isArray(context?.preferenceDefinitions)
    ? context.preferenceDefinitions
    : []
  ...
  const extStored = extName
    ? readExternalExtensionPreferenceRecord(getExternalExtensionPrefsKey(extName))
    : {}
  const cmdStored =
    extName && cmdName
      ? readExternalExtensionPreferenceRecord(
          getExternalExtensionCommandPrefsKey(extName, cmdName)
        )
      : {}
  ...
  const merged = { ...defaults, ...contextPrefs, ...stored }
...
export async function launchCommand(options: LaunchOptions): Promise<void> {
  ...
  try {
    ...
    throw new Error(
      `Failed to launch command "${options.name}": ${error instanceof Error ? error.message : "Unknown error"}`
    )
  }
}
