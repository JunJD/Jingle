...
export function listLauncherCommands(locale: AppLocale): LauncherIndexedCommand[] {
  return listAllLauncherCommandOwners().flatMap((owner) => {
    const commandDefinitionsByName = new Map(
      owner.commands.map((command) => [command.commandName, command] as const)
    )

    return owner.manifest.commands.map((command) => {
      const address =
      ...
      const commandDefinition = commandDefinitionsByName.get(command.name)
      if (!commandDefinition) {
        throw new Error(
          `Launcher command "${getLauncherCommandKey(address)}" is missing its runtime definition`
        )
      }
      ...
      return {
        ...
        requiresSearchArgument: commandDefinition.requiresSearchArgument === true,
