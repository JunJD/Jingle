...
const zhCN: SettingsCopy = {
  ...
  general: {
    ...
    followUpModeDescription: "在金果运行时，后续消息会加入队列，或引导当前运行。",
  ...
  appearance: {
    ...
    codeFont: "代码字体",
    codeTheme: "主题风格 ID",
    colorsDescription: "这些颜色会映射到金果的 semantic token。",
    colorsTitle: "颜色",
    contrast: "对比度",
    copied: "已复制主题",
    copyTheme: "复制主题",
  ...
  provider: {
    ...
    sectionTitle: "模型",
    secureStorageHint: "Provider secret 写入金果 auth.json，不再写入 ~/.jingle/.env。",
    showModels: "查看模型",
    retryModels: "重试模型列表",
    modelsColumn: "模型",
  ...
  extensions: {
    ...
    connectionDescription: "账号授权由金果管理；extension 只会收到运行所需的连接状态和 token。",
...
const enUS: SettingsCopy = {
  ...
  provider: {
    ...
    sectionTitle: "Models",
    secureStorageHint:
      "Provider secrets are stored in Jingle auth.json and are no longer written to ~/.jingle/.env.",
