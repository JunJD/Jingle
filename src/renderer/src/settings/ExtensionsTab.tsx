...
type ExtensionPreferenceEntry = readonly [string, Record<string, unknown>]
type ExtensionConnectionEntry = readonly [string, NativeExtensionResolvedConnection]
type ExtensionCommandPreferenceEntry = readonly [string, Record<string, unknown>]

function sortExtensionSettingsSchemas(
  schemas: InstalledNativeExtensionSettingsSchema[],
  locale: AppLocale
): InstalledNativeExtensionSettingsSchema[] {
  return schemas.toSorted((left, right) =>
...
function getSelectedExtensionNameAfterLoad(input: {
  currentExtensionName: string | null
  schemas: InstalledNativeExtensionSettingsSchema[]
  targetExtensionName: string | null
}): string | null {
  if (input.targetExtensionName) {
    return input.targetExtensionName
  }

  if (
    input.currentExtensionName &&
    input.schemas.some((schema) => schema.extName === input.currentExtensionName)
  ) {
    return input.currentExtensionName
  }

  const firstSchema = input.schemas[0]
  if (firstSchema) {
    return firstSchema.extName
  }

  return null
}

function loadExtensionPreferenceEntries(
  schemas: InstalledNativeExtensionSettingsSchema[]
): Promise<ExtensionPreferenceEntry[]> {
  return Promise.all(
    schemas.map((schema) =>
      window.api.nativeExtensions
        .getPreferences(schema.extName)
        .then((preferences) => [schema.extName, preferences] as const)
    )
  )
}

function loadExtensionConnectionEntries(
  schemas: InstalledNativeExtensionSettingsSchema[]
): Promise<ExtensionConnectionEntry[]> {
  return Promise.all(
    schemas
      .filter((schema) => schema.connection)
      .map((schema) =>
        window.api.nativeExtensions
          .getConnection(schema.extName)
          .then((connection) => [schema.extName, connection] as const)
      )
  )
}

function loadCommandPreferenceEntries(
  schemas: InstalledNativeExtensionSettingsSchema[]
): Promise<ExtensionCommandPreferenceEntry[]> {
  return Promise.all(
    schemas.flatMap((schema) =>
      schema.commands.map((command) =>
        window.api.nativeExtensions
          .getCommandPreferences(schema.extName, command.name)
          .then((preferences) => [`${schema.extName}:${command.name}`, preferences] as const)
      )
    )
  )
}

function getActiveModelOptions(models: ModelConfig[]): ModelOption[] {
  return models.reduce<ModelOption[]>((options, model) => {
...
export function ExtensionsTab(props: {
  ...
  const loadSchemas = useEffectEvent(
    async (targetExtensionName: string | null, signal: AbortSignal): Promise<void> => {
      ...
      const [extensionEntries, connectionEntries, commandEntries] = await Promise.all([
        loadExtensionPreferenceEntries(sortedSchemas),
        loadExtensionConnectionEntries(sortedSchemas),
        loadCommandPreferenceEntries(sortedSchemas)
      ])
      if (signal.aborted) {
        return
      }
  ...
  const modelOptions = useMemo(() => getActiveModelOptions(models), [models])
