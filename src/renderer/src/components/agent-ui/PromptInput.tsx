...
export function PromptInputActions(props: PromptInputActionsProps): React.JSX.Element {
  ...
  return (
    <div
      className={cn("flex min-w-0 items-center gap-[var(--jingle-gap-sm)]", className)}
      {...rest}
    />
  )
}

export interface PromptInputActionProps extends Omit<
...
export function PromptInputAction(props: PromptInputActionProps): React.JSX.Element {
  const { className, icon, label, tooltip, tooltipSide = "top", type = "button", ...rest } = props

  return (
    <IconButton
      className={cn(
        "size-[var(--jingle-prompt-input-action-size)] rounded-full border-0 bg-transparent text-muted-foreground hover:bg-background-secondary/80 hover:text-foreground disabled:cursor-default disabled:opacity-45",
        className
      )}
      label={label}
