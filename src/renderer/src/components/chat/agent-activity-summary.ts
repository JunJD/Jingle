...
import {
  ...
  type JingleAgentActivitySummaryCategory,
  type JingleAgentActivitySummaryProjection,
  type JingleAgentActivitySummaryToolInput,
...
export interface AgentActivitySummaryTool extends JingleAgentActivitySummaryToolInput {
...
function hasCategory(
  counts: JingleAgentActivitySummaryProjection["counts"],
  category: JingleAgentActivitySummaryCategory
): boolean {
  return counts[category] !== undefined
}

function getCategoryIcon(category: JingleAgentActivitySummaryCategory): AgentActivitySummaryIcon {
  switch (category) {
    case "command":
      return "command"
    case "file":
...
function getRunningTitle(copy: AppCopy, category: JingleAgentActivitySummaryCategory): string {
  switch (category) {
    case "command":
...
export function projectAgentActivityHeaderSummary(
  ...
  const projection = projectJingleAgentActivitySummary(tools)
  if (!projection) {
    return null
  }
