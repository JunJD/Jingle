...
interface ChatTodoProgressMetaProps {
  completedCount: number
  progress: number
  totalCount: number
}

function ChatTodoProgressMeta({
  completedCount,
  progress,
  totalCount
}: ChatTodoProgressMetaProps): React.JSX.Element {
  return (
    <>
...
export function ChatTodos({ todos }: ChatTodosProps): React.JSX.Element | null {
  ...
  return <ChatTodosList todos={todos} />
}

function ChatTodosList({ todos }: ChatTodosProps): React.JSX.Element {
  const { copy } = useI18n()
