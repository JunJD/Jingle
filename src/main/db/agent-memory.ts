...
type ListAgentMemoriesInput = ListOpenworkMemoriesInput & WorkspaceScopedInput
type ListAgentMemorySuggestionsInput = ListOpenworkSuggestionsInput & WorkspaceScopedInput
type CreateAgentMemoryInput = CreateOpenworkMemoryInput & WorkspaceScopedInput
type CreateAgentMemorySuggestionInput = CreateOpenworkMemorySuggestionInput & WorkspaceScopedInput
type AcceptAgentMemorySuggestionInput = AcceptOpenworkMemorySuggestionInput & WorkspaceScopedInput
type UpdateAgentMemoryInput = UpdateOpenworkMemoryInput & WorkspaceScopedInput

function normalizeScopeWorkspace(input: {
  scope: OpenworkMemoryRecord["scope"]
  workspaceKey?: string | null
}): { scope: OpenworkMemoryRecord["scope"]; workspaceKey: string | null } {
...
function mapMemory(row: AgentMemoryModel): OpenworkMemoryRecord {
  return {
    ...
    scope: row.scope as OpenworkMemoryRecord["scope"],
    source: row.source as OpenworkMemoryRecord["source"],
    status: row.status as OpenworkMemoryRecord["status"],
    type: row.type as OpenworkMemoryRecord["type"],
    updatedAt: toNumber(row.updatedAt),
    workspaceKey: row.workspaceKey
  }
}

function mapSuggestion(row: AgentMemorySuggestionModel): OpenworkMemorySuggestionRecord {
  return {
    content: row.content,
    createdAt: toNumber(row.createdAt),
    ...
    scope: row.scope as OpenworkMemorySuggestionRecord["scope"],
    sourceRunId: row.sourceRunId,
    status: row.status as OpenworkMemorySuggestionRecord["status"],
    suggestionId: row.suggestionId,
    threadId: row.threadId,
    type: row.type as OpenworkMemorySuggestionRecord["type"],
    updatedAt: toNumber(row.updatedAt),
    workspaceKey: row.workspaceKey
  }
}

function mapInclusion(row: AgentMemoryInclusionWithMemoryModel): OpenworkMemoryInclusionRecord {
  return {
    ...
    memoryId: row.memoryId,
    runId: row.runId,
    scope: row.memory.scope as OpenworkMemoryInclusionRecord["scope"],
    threadId: row.threadId,
    type: row.memory.type as OpenworkMemoryInclusionRecord["type"],
  ...
  const scopeWorkspace =
    input.scope !== undefined || input.workspaceKey !== undefined
      ? normalizeScopeWorkspace({
          scope: (input.scope ?? existing.scope) as OpenworkMemoryRecord["scope"],
...
export async function archiveAgentMemory(memoryId: string): Promise<OpenworkMemoryRecord> {
  const prisma = getPrismaClient()
  const row = await prisma.agentMemory.update({
    where: { memoryId },
    data: {
...
export async function listAgentMemoryInclusionsForRun(
  runId: string
): Promise<OpenworkMemoryInclusionRecord[]> {
  const prisma = getPrismaClient()
...
export async function getAgentMemory(memoryId: string): Promise<OpenworkMemoryRecord | null> {
  const prisma = getPrismaClient()
...
export async function getAgentMemorySuggestion(
  suggestionId: string
): Promise<OpenworkMemorySuggestionRecord | null> {
  const prisma = getPrismaClient()
