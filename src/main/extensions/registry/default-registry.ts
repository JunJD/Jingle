...
export function createDefaultExtensionRegistryService(): ExtensionRegistryService {
  ...
  const installedPackageIds = new Set(
    installedOwnerPackages
      .filter(
        (extensionPackage) =>
          extensionPackage.status === "loaded" && extensionPackage.enabled
      )
      .map((extensionPackage) => extensionPackage.id)
  )
