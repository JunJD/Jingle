...
const DESCRIPTOR_FILE_NAME = "openwork.extension.json"
...
export class InstalledExtensionProvider {
  ...
  private listExtensionVersionPackages(extensionId: string): ExtensionPackageDescriptor[] {
    ...
    const versionEntries = readdirSync(extensionRoot)
      .map((version) => join(extensionRoot, version))
      .filter(safeIsDirectory)
      .sort((left, right) => left.localeCompare(right))

    if (versionEntries.length === 0 && existsSync(join(extensionRoot, DESCRIPTOR_FILE_NAME))) {
    ...
    try {
      if (!existsSync(descriptorPath)) {
        return failedPackage({
          ...
          id: fallbackId,
          message: `Installed extension descriptor is missing: ${descriptorPath}`,
          rootDir: packageRoot
        })
      }
