...
function classifyCd(args: Array<string | null>): InvocationClassification {
  if (args.length > 1) {
    return {
      profile: "unknown_command",
      reason: "cd with multiple arguments requires user approval before it can run."
    }
  }
  ...
  if (!target) {
    return {
      profile: "read_only",
      reason: "cd without arguments only changes shell working directory."
    }
  }
  ...
  if (
    ...
    return {
      profile: "unknown_command",
      reason: "cd outside a workspace-relative directory requires user approval before it can run."
    }
  }

  return {
    profile: "read_only",
    reason: "cd only changes shell working directory."
  }
}
...
function classifyInvocation(invocation: CollectedCommand): InvocationClassification {
  ...
  if (name === "cd") {
    return classifyCd(invocation.args)
  }
