...
export interface MutationPredictorExecuteOptions {
  cwd?: string | null
}
...
export interface MutationPredictor {
  predictExecute(
    command: string,
    options?: MutationPredictorExecuteOptions
  ): Promise<MutationPrediction>
}
...
export class JustBashMutationPredictor implements MutationPredictor {
  ...
  async predictExecute(
    ...
    let mountCwd: string
    try {
      const sessionEnv = createExecuteCommandSessionEnv(this.workspacePath)
      cwd = sessionEnv.resolveCwd(options.cwd)
      mountCwd = sessionEnv.toMountCwd({
        cwd,
      ...
      return buildPrediction({
        ...
        status: "simulation_error",
        durationMs: performance.now() - startedAt,
        summary: `Simulation cwd is invalid: ${messageFromError(error)}; target files are unknown.`
      })
    }

    return this.runWorker(command, startedAt, mountCwd)
  }
