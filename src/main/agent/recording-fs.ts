...
export class RecordingFs implements IFileSystem {
  ...
  async collectChanges(): Promise<MutationPredictionChange[]> {
    ...
    for (const rootPath of Array.from(this.trackedRoots).sort()) {
      await this.captureTree(rootPath, afterSnapshots)
    }
  ...
  private async captureTree(
    ...
    if (stat.isDirectory) {
      ...
      for (const entry of entries) {
        await this.captureTree(this.inner.resolvePath(path, entry.name), snapshots, skipRoots)
      }
