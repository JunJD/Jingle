...
interface PendingRunBotAgentRequest {
  reject: (error: Error) => void
...
export class ExtensionRuntimeRendererBridge {
  ...
  releaseSession(sessionId: string): void {
    ...
    for (const [requestId, pending] of this.pendingRunBotAgentRequests) {
      if (pending.sessionId !== sessionId) {
        continue
      }
  ...
  handleRunBotAgentRequest(params: {
    request: ExtensionRuntimeRunBotAgentRequestEvent["request"]
    sessionId: string
  }): Promise<unknown> {
    const { request, sessionId } = params
    const owner = this.sessionOwners.get(sessionId)
    ...
    return new Promise((resolve, reject) => {
      this.pendingRunBotAgentRequests.set(pendingKey, {
        reject,
        resolve,
        sessionId
      })

      owner.send(EXTENSION_RUNTIME_RUN_BOT_AGENT_REQUEST_CHANNEL, {
        request,
        sessionId
      } satisfies ExtensionRuntimeRunBotAgentRequestEvent)
    })
  }
  ...
  completeRunBotAgentRequest(
    ...
    const pendingKey = getNavigationRequestKey(response.sessionId, response.requestId)
    const pending = this.pendingRunBotAgentRequests.get(pendingKey)
    if (!pending || pending.sessionId !== response.sessionId) {
      return false
    }

    this.pendingRunBotAgentRequests.delete(pendingKey)
    if (response.ok) {
