...
function resolveDesktopAutomationBinaryPath(): string | null {
  const candidates = [
    join(app.getAppPath(), "out/native/openwork-desktop-automation"),
    join(process.cwd(), "out/native/openwork-desktop-automation"),
    join(__dirname, "..", "native", "openwork-desktop-automation")
...
function resolveDesktopAutomationSwiftSourcePath(): string | null {
  const candidates = [
    join(app.getAppPath(), "src/native/openwork-desktop-automation.swift"),
    join(process.cwd(), "src/native/openwork-desktop-automation.swift"),
    join(__dirname, "..", "..", "src", "native", "openwork-desktop-automation.swift"),
    join(app.getAppPath(), "out/native/openwork-desktop-automation.swift"),
    join(process.cwd(), "out/native/openwork-desktop-automation.swift"),
    join(__dirname, "..", "native", "openwork-desktop-automation.swift")
  ]
...
function compileDesktopAutomationBinary(sourcePath: string): string {
  ...
  const binaryPath = join(nativeDir, "openwork-desktop-automation")
