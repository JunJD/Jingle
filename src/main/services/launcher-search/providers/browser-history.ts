...
async function copyChromiumHistoryDatabase(historyPath: string): Promise<string> {
  const tempDirectory = await fs.mkdtemp(path.join(os.tmpdir(), "openwork-browser-history-"))
...
async function scanChromiumBrowserProfiles(
  ...
  const profileDirectoryNames = entries
    .filter((entry) => entry.isDirectory())
    .map((entry) => entry.name)
    .filter((entryName) => entryName === "Default" || entryName.startsWith("Profile "))
