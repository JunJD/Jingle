  ...
  return {
    ...
    async flush() {
      const flushUntilIdle = async (): Promise<void> => {
        const dirtyJobCount = state.dirtyJobs.size
        const scheduledJobCount = state.scheduledJobs.size
        if (!state.timer && !state.drainQueued && dirtyJobCount === 0 && scheduledJobCount === 0) {
          return
        }
        ...
        } else if ((dirtyJobCount > 0 || scheduledJobCount > 0) && !state.drainQueued) {
          queueDrain({ flush: true })
        }
