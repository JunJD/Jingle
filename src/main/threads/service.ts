...
function readMessageStateVersionForCheckpoint(checkpoint: CheckpointTuple): string | null {
  const version = checkpoint.checkpoint.channel_versions.messages
  if (version === undefined) {
    return null
  }
  if (typeof version !== "string") {
    throw new Error(
      `[Threads] Checkpoint "${checkpoint.checkpoint.id}" has non-string messages channel version "${String(version)}".`
    )
  }
  return version
}
...
export class ThreadsService {
  ...
  async cloneUntilMessage(sourceThreadId: string, messageId: string): Promise<Thread> {
    ...
    const latestMessagesVersion = latest ? readMessageStateVersionForCheckpoint(latest) : null
    ...
    while (cursor.parentConfig) {
      ...
      const parentMessagesVersion = readMessageStateVersionForCheckpoint(parent)
