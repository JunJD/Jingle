...
const invokedName = path.basename(process.argv[1] || "openwork")
...
function usage() {
  const command = invokedName === "jl" ? "jl" : "openwork"
  console.log(`
${command} - A tactical agent interface for deepagentsjs

Usage:
  ${command}                         Launch the application
  jl trace list                      List local agent traces
  jl trace inspect latest            Inspect the latest trace
  jl trace inspect <traceId>         Inspect a trace timeline
...
function bindSqlParams(sql, params) {
  ...
  return sql.replace(/\?/g, () => {
    if (index >= params.length) {
      throw new Error("Missing SQL parameter for trace CLI query.")
    }
...
if (invokedName === "jl" && args[0] === "trace") {
  try {
...
if (args[0] === "trace") {
  console.error("Trace commands are available as `jl trace`, not `openwork trace`.")
  process.exit(1)
}

process.title = invokedName === "jl" ? "jl" : "openwork"

if (args.includes("--version") || args.includes("-v")) {
  const { version } = require("../package.json")
  console.log(`${invokedName} v${version}`)
  process.exit(0)
}
