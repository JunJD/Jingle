...
```bash
node .agents/skills/ljg-card/assets/capture.js <html> <png> <width> <height> [fullpage]
```

依赖：`.agents/skills/ljg-card/node_modules/` 中的 playwright。如报错：

```bash
cd .agents/skills/ljg-card && npm install playwright && npx playwright install chromium
```
